#!/usr/bin/env bash
# new-bench.sh — stamp out a standard per-kata bench/bench.sh.
#
# The benchmark harness is ~180 lines that are identical across katas apart
# from six values. Copy-pasting it by hand is how bench.sh files drift out of
# sync with scripts/bench-lib.sh, so generate it instead.
#
# Usage:
#   scripts/new-bench.sh \
#       --dir leetcode/301-400/347-top-k-frequent-elements \
#       --stem top_k_frequent \
#       --id 347 --slug top-k-frequent-elements --group 301-400 \
#       --title "Top K Frequent Elements" \
#       --workload "build N once, then M punches; sink=..."
#
# Writes <dir>/bench/bench.sh (refuses to clobber unless --force) and creates
# <dir>/bench/ plus <dir>/bench/go-seq/go.mod. The five mirror sources
# (<stem>.kara/.rs/.c/.py and go-seq/main.go) are per-kata algorithm code and
# are still written by hand.

set -euo pipefail

DIR="" STEM="" ID="" SLUG="" GROUP="" TITLE="" WORKLOAD="" FORCE=0
...
while [ $# -gt 0 ]; do
    case "$1" in
        ...
        --force)    FORCE=1; shift ;;
        *) echo "new-bench: unknown arg '$1'" >&2; exit 2 ;;
    esac
done

for req in DIR STEM ID SLUG GROUP TITLE WORKLOAD; do
    if [ -z "${!req}" ]; then
        echo "new-bench: missing --$(echo "$req" | tr '[:upper:]' '[:lower:]')" >&2
        exit 2
    fi
done

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
BENCH="$ROOT/$DIR/bench"
OUT="$BENCH/bench.sh"

if [ -e "$OUT" ] && [ "$FORCE" != "1" ]; then
    echo "new-bench: $OUT exists (pass --force to overwrite)" >&2
    exit 1
fi

# Depth from bench/ back to the repo root, so the generated script can source
# bench-lib.sh regardless of how deeply the kata is nested.
...
mkdir -p "$BENCH/go-seq"
if [ ! -e "$BENCH/go-seq/go.mod" ]; then
    printf 'module %s_bench\n\ngo 1.24\n' "$STEM" >"$BENCH/go-seq/go.mod"
fi

cat >"$OUT" <<EOF
#!/usr/bin/env bash
# Wall-clock comparison across implementations of LeetCode #${ID}.
...
bench_begin id=${ID} slug=${SLUG} group=${GROUP} \\
    title="${TITLE}" \\
    workload="${WORKLOAD}" sink="\$expected"
...
# NOTE: measure the cold compile WITHOUT touching target/\${STEM}_kara.
# That path holds the KARAC_AUTO_PAR=0 binary the seq runtime lane
# benchmarks; a plain \`karac build\` here is an AUTO-PAR build, so moving
# it into place left the next run's build_kara freshness check satisfied
# and the seq lane silently measuring an auto-par binary.
rm -f "\${STEM}"
