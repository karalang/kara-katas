#!/usr/bin/env bash
# Wall-clock comparison across implementations of LeetCode #11.
# See ../README.md § Benchmarks for what these numbers mean.
#
# Two-lane kata (BENCH.md § Implicit auto-par): karac's auto-par-on-
# reduction recognizes the `sum = sum + max_area_off(...)` accumulator in
# main's K=10M loop and emits a `karac_par_reduce` dispatch by default.
# We build TWO kara binaries:
#
#   container_kara_seq — KARAC_AUTO_PAR=0, single-threaded.
#                        Within-lane comparator vs rust / c / go.
#   container_kara     — default karac build, auto-par on.
...
bench_begin id=11 slug=container-with-most-water group=1-100 \
    title="Container With Most Water" \
    workload="kara/rust/c/go K=10^7 reduction over max-area; py K=10^6 scaled-down" \
    sink="$expected"
...
rt_cmd --lang kara --approach container --lane seq --mode codegen \
    --name "kara ${STEM} (seq, KARAC_AUTO_PAR=0)" --cmd "./target/${STEM}_kara_seq"
rt_cmd --lang rust --approach container --lane seq --mode native \
    --name "rust ${STEM}" --cmd "./target/${STEM}"
rt_cmd --lang rust_ovf --approach container --lane seq --mode native \
    --name "rust container (overflow-checks=on, equal-safety)" --cmd "./target/${STEM}_ovf"
rt_cmd --lang c --approach container --lane seq --mode native \
    --name "c    ${STEM}" --cmd "./target/${STEM}_c"
rt_cmd --lang go --approach container --lane seq --mode native \
...
rt_cmd --lang kara --approach container --lane par --mode codegen \
    --name "kara ${STEM} (auto-par default)" --cmd "./target/${STEM}_kara"
rt_cmd --lang c --approach container --lane par --mode native \
    --name "c    ${STEM} (pthreads — metal floor)" --cmd "./target/${STEM}_c_par"
rt_cmd --lang rust --approach container --lane par --mode native \
    --name "rust ${STEM} (rayon par_iter)" --cmd "./target/${STEM}_rayon"
rt_cmd --lang go --approach container --lane par --mode native \
...
rt_cmd --lang python --approach container --lane seq --mode interp \
...
ce_cmd --lang kara --approach container --mode codegen \
...
ce_cmd --lang rust --approach container --mode native \
    --prepare "rm -f target/${STEM}" \
    --name "rustc -O ${STEM}.rs" --cmd "rustc -O ${STEM}.rs -o target/${STEM}"
ce_cmd --lang c --approach container --mode native \
...
echo
echo "=== binary size ==="
size_put --lang kara --approach container --lane seq --mode codegen --path "target/${STEM}_kara_seq"
size_put --lang kara --approach container --lane par --mode codegen --path "target/${STEM}_kara"
size_put --lang c    --approach container --lane par --mode native  --path "target/${STEM}_c_par"
size_put --lang rust --approach container --lane par --mode native  --path "target/${STEM}_rayon"
size_put --lang go   --approach container --lane par --mode native  --path "target/${STEM}_go_par"
size_put --lang rust --approach container --lane seq --mode native  --path "target/${STEM}"
size_put --lang c    --approach container --lane seq --mode native  --path "target/${STEM}_c"
size_put --lang go   --approach container --lane seq --mode native  --path "target/${STEM}_go_seq"

echo
echo "=== runtime memory (peak) ==="
mem_put --lang kara --approach container --lane seq --mode codegen --bytes "$(mem_peak ./target/${STEM}_kara_seq)"
mem_put --lang kara --approach container --lane par --mode codegen --bytes "$(mem_peak ./target/${STEM}_kara)"
mem_put --lang c    --approach container --lane par --mode native  --bytes "$(mem_peak ./target/${STEM}_c_par)"
mem_put --lang rust --approach container --lane par --mode native  --bytes "$(mem_peak ./target/${STEM}_rayon)"
mem_put --lang go   --approach container --lane par --mode native  --bytes "$(mem_peak ./target/${STEM}_go_par)"
mem_put --lang rust --approach container --lane seq --mode native  --bytes "$(mem_peak ./target/${STEM})"
mem_put --lang c    --approach container --lane seq --mode native  --bytes "$(mem_peak ./target/${STEM}_c)"
mem_put --lang go   --approach container --lane seq --mode native  --bytes "$(mem_peak ./target/${STEM}_go_seq)"
...
for src in "${STEM}.kara"; do
    ...
    cmem_put --lang kara --approach container --mode codegen --bytes "$bytes"
done
for src in "${STEM}.rs"; do
    out="target/$(basename "$src" .rs)"
    rm -f "$out"
    cmem_put --lang rust --approach container --mode native --bytes "$(mem_peak rustc -O "$src" -o "$out")"
done
for src in "${STEM}.c"; do
    out="target/$(basename "$src" .c)_c"
    rm -f "$out"
    cmem_put --lang c --approach container --mode native --bytes "$(mem_peak clang -O3 "$src" -o "$out")"
done
