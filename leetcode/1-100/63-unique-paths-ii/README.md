...
> **⚠️ M5 re-bench (2026-07-08) contradicts the container parity above — ledger `B-2026-07-08-3` (open).** Re-run on the corpus's Apple M5 Pro reference machine (arm64, clang 21 / rustc 1.95), kāra is **1.18× behind both Rust and C**, not ahead: kāra 190.4 ± 10.9 ms vs rust 161.0 ± 1.1 ms / c 161.3 ± 0.6 ms (go 282.9 ms). On deterministic instructions-retired the gap is **1.36×** (kāra 5.75B vs 4.24B/4.21B). Root-caused to a codegen strength-reduction miss: kāra rebuilds the obstacle-predicate dividend `i*7+c*3+k` from components (with overflow checks) every cell, where LLVM maintains it as one running induction variable — so the container's `0.95× Rust` was an x86/container artifact, **not** true M5 parity. This tables' numbers are the stale container snapshot pending an M5 rewrite; see `B-2026-07-08-3` for the full instruction/cycle breakdown and equal-safety control.
