#!/usr/bin/env bash
# Wall-clock comparison across implementations of LeetCode #63.
# See ../README.md § Benchmarks for what these numbers mean.
#
# Seq-only kata: the loop sweeps grid shapes (m = 2 + k%32, n = 2 + (k/32)%32),
# fills a rolling DP array (`dp[c] += dp[c-1]`, sized to the real column count n
# — obstacles break #62's axis symmetry, so no swap to the smaller side) with an
# inline obstacle predicate ((i*7 + c*3 + k) % 13 == 0), and folds each path
# count into a rolling polynomial hash 1_000_000× per run. The hash
...
# on a per-core codegen-quality basis. Same discipline as kata #55/#62.
#
...
# (obstacle pattern also threaded on k) into a rolling hash
...
# (sink 785636203) — timed separately, not cross-checked.
expected="583461340"
mismatch=""
for pair in \
...
bench_begin id=63 slug=unique-paths-ii group=1-100 \
    title="Unique Paths II" \
    workload="K=1_000_000 rolling obstacle-DP over swept m×n grids / polynomial-hash sink" \
    sink="$expected"

echo "=== runtime — seq lane (apples-to-apples, single-threaded) ==="
rt_begin --warmup 5 --runs 30
rt_cmd --lang kara --approach unique_paths_ii --lane seq --mode codegen \
    --name "kara ${STEM}" --cmd "./target/${STEM}_kara"
rt_cmd --lang rust --approach unique_paths_ii --lane seq --mode native \
    --name "rust ${STEM}" --cmd "./target/${STEM}"
ovf_rt_cmds "${STEM}" "unique_paths_ii" seq
rt_cmd --lang c --approach unique_paths_ii --lane seq --mode native \
...
rt_cmd --lang python --approach unique_paths_ii --lane seq --mode interp \
...
ce_cmd --lang kara --approach unique_paths_ii --mode codegen \
...
ce_cmd --lang rust --approach unique_paths_ii --mode native \
    --prepare "rm -f target/${STEM}" \
    --name "rustc -O ${STEM}.rs" --cmd "rustc -O ${STEM}.rs -o target/${STEM}"
ce_cmd --lang c --approach unique_paths_ii --mode native \
...
echo
echo "=== binary size ==="
size_put --lang kara --approach unique_paths_ii --lane seq --mode codegen --path "target/${STEM}_kara"
size_put --lang rust --approach unique_paths_ii --lane seq --mode native  --path "target/${STEM}"
size_put --lang c    --approach unique_paths_ii --lane seq --mode native  --path "target/${STEM}_c"
size_put --lang go   --approach unique_paths_ii --lane seq --mode native  --path "target/${STEM}_go_seq"

echo
echo "=== runtime memory (peak) ==="
mem_put --lang kara --approach unique_paths_ii --lane seq --mode codegen --bytes "$(mem_peak ./target/${STEM}_kara)"
mem_put --lang rust --approach unique_paths_ii --lane seq --mode native  --bytes "$(mem_peak ./target/${STEM})"
mem_put --lang c    --approach unique_paths_ii --lane seq --mode native  --bytes "$(mem_peak ./target/${STEM}_c)"
mem_put --lang go   --approach unique_paths_ii --lane seq --mode native  --bytes "$(mem_peak ./target/${STEM}_go_seq)"
