...
#
# Seq-only kata: the Dutch National Flag one-pass sort (the kata's ★) is run over
# an n=500 {0,1,2} array allocated ONCE and reused — each of K=200_000 iterations
# refills it in place with a k-dependent pattern, sorts it in place, and folds the
# sorted result into a rolling hash. Allocating once and refilling in place (not
# growing a fresh Vec per iteration, kata #73's regime) keeps the measured work the
# SORT's data-dependent branches and swaps. The loop-carried hash is NOT a
...
# included as the faithful like-for-like (kata #69's discipline). Python runs
# 1/10th K (a pure-Python sort loop is ~150 s at full scale) — timed separately,
# NOT cross-checked. Same discipline as kata #74.
#
...
# timing. Over K=200_000 iterations the fold is a fixed constant; Python runs a
...
bench_begin id=75 slug=sort-colors group=1-100 \
    title="Sort Colors" \
    workload="K=200_000 Dutch National Flag one-pass sort of a build-once, refilled-in-place n=500 {0,1,2} array / polynomial-hash sink" \
    sink="$expected"
...
echo
echo "=== binary size ==="
size_put --lang kara --approach sort_colors --lane seq --mode codegen --path "target/${STEM}_kara"
...
mem_put --lang kara --approach sort_colors --lane seq --mode codegen --bytes "$(mem_peak ./target/${STEM}_kara)"
