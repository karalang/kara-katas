...
> **Follow-up (compiler improvement, `23287e52`):** the numbers above predate an extension to karac's automatic loop-fill pre-sizing pass — it now recognizes `while d <= n { … }` counted fills (previously only `<`). With it, the solver's `digits` fill auto-pre-sizes and the **factorial seq lane drops ~1.35× with no source change** (measured 94.6 → 69.9 ms on a re-run), narrowing the gap to the C/Go floor. The `fact` accumulator still grows (it seeds with a pre-loop `push`, a case the pass doesn't yet cover). These tables are the pre-improvement snapshot; a re-benchmark once that compiler build ships would show the tighter factorial lane.
