...
> **A karac auto-par bug found here (and worked around):** the first cut fed the rotated `(n, k)` cases through captured fixed-size `Array[i64, 9]` lookup tables. Under the default auto-par build those tables miscompile — a worker indexing `atab[k % m]` with a loop-derived index traps with `vec index out of bounds`, while a constant index and `Vec[i64]` tables are both fine. This is a follow-on gap in the by-pointer array-capture fix `c3050fc8` (ledger `B-2026-06-15-3`): the captured array's *pointer* reaches the worker but the *length* used by the dynamic-index bounds check does not. The bench uses `Vec[i64]` tables (the idiomatic choice anyway); the minimal repro is preserved for a dedicated karac fix slice.
