#!/usr/bin/env bash
# Wall-clock comparison across implementations of LeetCode #57 (Insert
# Interval).
...
#
#   insert_interval_kara_seq — KARAC_AUTO_PAR=0, single-threaded.
#                              Within-lane comparator vs rust / c / go.
#   insert_interval_kara     — default karac build, auto-par on (if it
#                              fires). Reported separately so the
#                              production-default Kara behavior stays
#                              visible.
#
# Unlike kata 56 (Merge Intervals), this kata's input is ALREADY sorted
# and disjoint — #57's precondition — so the per-call work is a pure O(N)
# three-phase sweep with NO sort. That isolates the tuple index +
# field-access + Vec.push allocation path from the sort-dispatch story:
# same M/N/K shape as kata 56, but no karac_vec_sort_by in the picture.
#
...
bench_begin id=57 slug=insert-interval group=1-100 \
    title="Insert Interval" workload="K=1M insert passes (py K=100k scaled-down)" \
    sink="$expected"
...
rt_cmd --lang kara --approach insert_interval --lane par --mode codegen \
    --name "kara ${STEM} (auto-par default)" --cmd "./target/${STEM}_kara"
rt_cmd --lang c --approach insert_interval --lane par --mode native \
    --name "c    ${STEM} (pthreads — metal floor)" --cmd "./target/${STEM}_c_par"
rt_cmd --lang rust --approach insert_interval --lane par --mode native \
    --name "rust ${STEM} (rayon par_iter)" --cmd "./target/${STEM}_rayon"
rt_cmd --lang go --approach insert_interval --lane par --mode native \
...
rt_cmd --lang python --approach insert_interval --lane seq --mode interp \
...
ce_cmd --lang kara --approach insert_interval --mode codegen \
...
ce_cmd --lang rust --approach insert_interval --mode native \
    --prepare "rm -f target/${STEM}" \
    --name "rustc -O ${STEM}.rs" --cmd "rustc -O ${STEM}.rs -o target/${STEM}"
ce_cmd --lang c --approach insert_interval --mode native \
...
size_put --lang go   --approach insert_interval --lane par --mode native  --path "target/${STEM}_go_par"
size_put --lang rust --approach insert_interval --lane seq --mode native  --path "target/${STEM}"
size_put --lang c    --approach insert_interval --lane seq --mode native  --path "target/${STEM}_c"
size_put --lang go   --approach insert_interval --lane seq --mode native  --path "target/${STEM}_go_seq"
...
mem_put --lang go   --approach insert_interval --lane par --mode native  --bytes "$(mem_peak ./target/${STEM}_go_par)"
mem_put --lang rust --approach insert_interval --lane seq --mode native  --bytes "$(mem_peak ./target/${STEM})"
mem_put --lang c    --approach insert_interval --lane seq --mode native  --bytes "$(mem_peak ./target/${STEM}_c)"
mem_put --lang go   --approach insert_interval --lane seq --mode native  --bytes "$(mem_peak ./target/${STEM}_go_seq)"
...
for src in "${STEM}.kara"; do
    ...
    cmem_put --lang kara --approach insert_interval --mode codegen --bytes "$bytes"
done
for src in "${STEM}.rs"; do
    out="target/$(basename "$src" .rs)"
    rm -f "$out"
    cmem_put --lang rust --approach insert_interval --mode native --bytes "$(mem_peak rustc -O "$src" -o "$out")"
done
for src in "${STEM}.c"; do
    out="target/$(basename "$src" .c)_c"
    rm -f "$out"
    cmem_put --lang c --approach insert_interval --mode native --bytes "$(mem_peak clang -O3 "$src" -o "$out")"
done
