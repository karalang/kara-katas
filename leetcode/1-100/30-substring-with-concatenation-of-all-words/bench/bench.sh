#!/usr/bin/env bash
# Wall-clock comparison across implementations of LeetCode #30.
...
build_rust() {
    local src="$1"
    local out="target/$(basename "$src" .rs)_rs"
...
bench_begin id=30 slug=substring-with-concatenation-of-all-words group=1-100 \
    title="Substring with Concatenation of All Words" \
    workload="50K-word text, 40 sliding-window searches (4-word targets)" \
    sink="$expected"
...
rt_cmd --lang rust --approach "$STEM" --lane seq --mode native \
    --name "rust ${STEM}" --cmd "./target/${STEM}_rs"
ovf_rt_cmds "$STEM" "$STEM" seq
...
ce_cmd --lang rust --approach "$STEM" --mode native \
    --prepare "rm -f target/${STEM}_rs" \
    --name "rustc -O ${STEM}.rs" --cmd "rustc -O ${STEM}.rs -o target/${STEM}_rs"
...
size_put --lang rust --approach "$STEM" --lane seq --mode native  --path "target/${STEM}_rs"
...
mem_put --lang rust --approach "$STEM" --lane seq --mode native  --bytes "$(mem_peak ./target/${STEM}_rs)"
