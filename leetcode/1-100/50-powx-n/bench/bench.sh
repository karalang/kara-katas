#!/usr/bin/env bash
# Wall-clock comparison across implementations of LeetCode #50.
...
bench_begin id=50 slug=powx-n group=1-100 \
    title="Pow(x, n)" workload="N=8M recursive fast-powers, x in [1,2), |n|<=64, f64-bits sum sink" \
    sink="$expected"
