#!/usr/bin/env bash
# Wall-clock comparison across implementations of LeetCode #15 (3Sum).
...
#
#   three_sum_kara_seq — KARAC_AUTO_PAR=0, single-threaded.
#                        Within-lane comparator vs rust / c / go.
#   three_sum_kara     — default karac build, auto-par on (if it fires).
...
bench_begin id=15 slug=3sum group=1-100 \
    title="3Sum" workload="distinct triplets K=1M" \
    sink="$expected"
...
rt_cmd --lang kara --approach three_sum --lane seq --mode codegen \
...
rt_cmd --lang kara --approach three_sum --lane par --mode codegen \
    --name "kara ${STEM} (auto-par default)" --cmd "./target/${STEM}_kara"
rt_cmd --lang c --approach three_sum --lane par --mode native \
    --name "c    ${STEM} (pthreads — metal floor)" --cmd "./target/${STEM}_c_par"
rt_cmd --lang rust --approach three_sum --lane par --mode native \
    --name "rust ${STEM} (rayon par_iter)" --cmd "./target/${STEM}_rayon"
rt_cmd --lang go --approach three_sum --lane par --mode native \
...
rt_cmd --lang python --approach three_sum --lane seq --mode interp \
...
ce_cmd --lang kara --approach three_sum --mode codegen \
...
ce_cmd --lang rust --approach three_sum --mode native \
    --prepare "rm -f target/${STEM}" \
    --name "rustc -O ${STEM}.rs" --cmd "rustc -O ${STEM}.rs -o target/${STEM}"
ce_cmd --lang c --approach three_sum --mode native \
...
echo
echo "=== binary size ==="
size_put --lang kara --approach three_sum --lane seq --mode codegen --path "target/${STEM}_kara_seq"
size_put --lang kara --approach three_sum --lane par --mode codegen --path "target/${STEM}_kara"
size_put --lang c    --approach three_sum --lane par --mode native  --path "target/${STEM}_c_par"
size_put --lang rust --approach three_sum --lane par --mode native  --path "target/${STEM}_rayon"
size_put --lang go   --approach three_sum --lane par --mode native  --path "target/${STEM}_go_par"
size_put --lang rust --approach three_sum --lane seq --mode native  --path "target/${STEM}"
size_put --lang c    --approach three_sum --lane seq --mode native  --path "target/${STEM}_c"
size_put --lang go   --approach three_sum --lane seq --mode native  --path "target/${STEM}_go_seq"

echo
echo "=== runtime memory (peak) ==="
mem_put --lang kara --approach three_sum --lane seq --mode codegen --bytes "$(mem_peak ./target/${STEM}_kara_seq)"
mem_put --lang kara --approach three_sum --lane par --mode codegen --bytes "$(mem_peak ./target/${STEM}_kara)"
mem_put --lang c    --approach three_sum --lane par --mode native  --bytes "$(mem_peak ./target/${STEM}_c_par)"
mem_put --lang rust --approach three_sum --lane par --mode native  --bytes "$(mem_peak ./target/${STEM}_rayon)"
mem_put --lang go   --approach three_sum --lane par --mode native  --bytes "$(mem_peak ./target/${STEM}_go_par)"
mem_put --lang rust --approach three_sum --lane seq --mode native  --bytes "$(mem_peak ./target/${STEM})"
mem_put --lang c    --approach three_sum --lane seq --mode native  --bytes "$(mem_peak ./target/${STEM}_c)"
mem_put --lang go   --approach three_sum --lane seq --mode native  --bytes "$(mem_peak ./target/${STEM}_go_seq)"
