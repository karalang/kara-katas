...
**Profiled 2026-07-04 — the cause is auto-vectorization, not nested indexing.** The first cut of this README guessed the gap was nested `Vec[Vec[i64]]` double-indirection; a profiling pass **refuted that**. Decomposing the workload by rewriting it flat (`Vec[i64]`, index `i·n+j`) and with a reused buffer (zero per-iter allocation) shows the ratio is stable regardless of representation or allocation:

| variant | Kāra | Rust | ratio |
|---|---|---|---|
| nested `Vec[Vec]` (`n+1` allocs/iter) | 114.0 ms | 78.2 ms | 1.46× |
| flat `Vec[i64]` (1 alloc/iter) | 51.3 ms | 33.7 ms | 1.52× |
| flat, reused buffer (0 allocs/iter) | 39.0 ms | 26.3 ms | 1.48× |

Nesting and allocation each move the *absolute* times a lot (nested → flat is ~2.2× on **both** sides — nesting is expensive for everyone) but leave the **Kāra:Rust ratio pinned at ~1.5×**. Disassembly (`otool -tvV`) finds the real cause: **the Rust binary contains 2,589 NEON vector instructions; the Kāra binary contains 0.** Rust auto-vectorizes the checksum reduction (and the fill); Kāra emits fully scalar code. Isolation tests ruled out every other suspect one at a time — dropping the `i·n` index multiply (running linear index), and dropping the bounds check (`get_unchecked`), each left Kāra at 0 vector ops and ~1.5× behind. So the gap is a **general auto-vectorization gap in karac's LLVM backend** on tight numeric loops, not anything specific to this kata's nested grid. That makes it a broad codegen lever (it would help any vectorizable kernel across the corpus), tracked in the karac spike [`codegen-autovectorization.md`](https://github.com/karalang/kara/blob/main/docs/spikes/codegen-autovectorization.md). C leads Rust by only 1.16×, both vectorizing; Kāra is the outlier that stays scalar.
...
Same rationale as [`1-two-sum/README.md § Why this kata is in the harness`](../1-two-sum/README.md#why-this-kata-is-in-the-harness): Rust is Kāra's semantic peer, so the headline ratio is the codegen-vs-Rust gap. On this kata that ratio is **1.46× (seq) / 1.60× (auto-par) in Rust's favor** — and that's the point of keeping it: unlike the flat-allocation katas where Kāra ties Rust, this numeric-loop workload exposes a standing codegen gap. The profiling pass above pinned it to **auto-vectorization** — Rust emits 2,589 SIMD instructions, Kāra emits 0 — a broad optimization target (it would help any vectorizable kernel), not a quirk of this kata. C calibrates the LLVM floor, Go is the cross-runtime data point (and here shows GC-allocation contention defeating its own goroutine parallelism), Python is the ergonomic foil.
