/* LeetCode #16 — C pthreads-parallel mirror (par-lane BARE-METAL FLOOR, 3Sum Closest).
 * Same sort + two-pointer three_sum_closest; the K=1M reduction split across a
 * fixed pool of _SC_NPROCESSORS_ONLN pthreads (spawn once, chunk, join+merge).
 * Raw OS threads, no runtime — the ceiling auto-par is measured against. Sink
 * matches the kara/rust/c/go mirrors.
 * Build: clang -O3 three_sum_closest_par.c -o … -lpthread */
#include <pthread.h>
#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>
...
static void *worker(void *arg) {
    ...
    for (int64_t k = wk->start; k < wk->end; k++) {
        ...
        s += three_sum_closest(wk->sets[idx], N_VALUES, target_for(idx));
    }
    wk->partial = s;
    return NULL;
}
