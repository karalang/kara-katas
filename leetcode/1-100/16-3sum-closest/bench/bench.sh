#!/usr/bin/env bash
# Wall-clock comparison across implementations of LeetCode #16 (3Sum Closest).
...
# reduction recognizes the `sum = sum + three_sum_closest(...)` accumulator
# in main's K=1M loop and may emit a `karac_par_reduce` dispatch by default.
# We build TWO kara binaries:
#
#   three_sum_closest_kara_seq — KARAC_AUTO_PAR=0, single-threaded.
#                                Within-lane comparator vs rust / c / go.
#   three_sum_closest_kara     — default karac build, auto-par on (if it
...
bench_begin id=16 slug=3sum-closest group=1-100 \
    title="3Sum Closest" workload="closest-to-target K=1M" \
    sink="$expected"
...
rt_cmd --lang kara --approach three_sum_closest --lane seq --mode codegen \
    --name "kara ${STEM} (seq, KARAC_AUTO_PAR=0)" --cmd "./target/${STEM}_kara_seq"
rt_cmd --lang rust --approach three_sum_closest --lane seq --mode native \
    --name "rust ${STEM}" --cmd "./target/${STEM}"
rt_cmd --lang rust_ovf --approach three_sum_closest --lane seq --mode native \
    --name "rust three_sum_closest (overflow-checks=on, equal-safety)" --cmd "./target/${STEM}_ovf"
rt_cmd --lang c --approach three_sum_closest --lane seq --mode native \
    --name "c    ${STEM}" --cmd "./target/${STEM}_c"
rt_cmd --lang go --approach three_sum_closest --lane seq --mode native \
...
# recognizes the K=1M reduction in `main` it emits a karac_par_reduce
...
rt_cmd --lang kara --approach three_sum_closest --lane par --mode codegen \
    --name "kara ${STEM} (auto-par default)" --cmd "./target/${STEM}_kara"
rt_cmd --lang c --approach three_sum_closest --lane par --mode native \
    --name "c    ${STEM} (pthreads — metal floor)" --cmd "./target/${STEM}_c_par"
rt_cmd --lang rust --approach three_sum_closest --lane par --mode native \
    --name "rust ${STEM} (rayon par_iter)" --cmd "./target/${STEM}_rayon"
rt_cmd --lang go --approach three_sum_closest --lane par --mode native \
...
rt_cmd --lang python --approach three_sum_closest --lane seq --mode interp \
...
ce_cmd --lang kara --approach three_sum_closest --mode codegen \
...
ce_cmd --lang rust --approach three_sum_closest --mode native \
    --prepare "rm -f target/${STEM}" \
    --name "rustc -O ${STEM}.rs" --cmd "rustc -O ${STEM}.rs -o target/${STEM}"
ce_cmd --lang c --approach three_sum_closest --mode native \
...
echo
echo "=== binary size ==="
size_put --lang kara --approach three_sum_closest --lane seq --mode codegen --path "target/${STEM}_kara_seq"
size_put --lang kara --approach three_sum_closest --lane par --mode codegen --path "target/${STEM}_kara"
size_put --lang c    --approach three_sum_closest --lane par --mode native  --path "target/${STEM}_c_par"
size_put --lang rust --approach three_sum_closest --lane par --mode native  --path "target/${STEM}_rayon"
size_put --lang go   --approach three_sum_closest --lane par --mode native  --path "target/${STEM}_go_par"
size_put --lang rust --approach three_sum_closest --lane seq --mode native  --path "target/${STEM}"
size_put --lang c    --approach three_sum_closest --lane seq --mode native  --path "target/${STEM}_c"
size_put --lang go   --approach three_sum_closest --lane seq --mode native  --path "target/${STEM}_go_seq"

echo
echo "=== runtime memory (peak) ==="
mem_put --lang kara --approach three_sum_closest --lane seq --mode codegen --bytes "$(mem_peak ./target/${STEM}_kara_seq)"
mem_put --lang kara --approach three_sum_closest --lane par --mode codegen --bytes "$(mem_peak ./target/${STEM}_kara)"
mem_put --lang c    --approach three_sum_closest --lane par --mode native  --bytes "$(mem_peak ./target/${STEM}_c_par)"
mem_put --lang rust --approach three_sum_closest --lane par --mode native  --bytes "$(mem_peak ./target/${STEM}_rayon)"
mem_put --lang go   --approach three_sum_closest --lane par --mode native  --bytes "$(mem_peak ./target/${STEM}_go_par)"
mem_put --lang rust --approach three_sum_closest --lane seq --mode native  --bytes "$(mem_peak ./target/${STEM})"
mem_put --lang c    --approach three_sum_closest --lane seq --mode native  --bytes "$(mem_peak ./target/${STEM}_c)"
mem_put --lang go   --approach three_sum_closest --lane seq --mode native  --bytes "$(mem_peak ./target/${STEM}_go_seq)"
