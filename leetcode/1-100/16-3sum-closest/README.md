...
- **`Vec.from_slice(Slice[i64])` + in-place `sort_by`** — same shape as kata [#15](../15-3sum/) and kata [#1665](../../1601-1700/1665-minimum-initial-energy-to-finish-tasks/): copy `nums` into an owned `Vec[i64]`, then `s.sort_by(|a, b| a.cmp(b))` to monomorphize the comparator. Verified to compile and run under both `karac run` and `karac build`.
...
- **Inline `abs_i64` helper** — the hot loop calls `abs_i64(sum - target) < abs_i64(best - target)` twice per non-exact-hit step. The helper is a one-line `if x < 0 { -x } else { x }`, exercising the branchy-abs lowering path; the seq-lane bench below is the production indicator for whether that lowering matches Rust's `.abs()` intrinsic.
...
This is the same sort-bound shape as kata 15, but the inner body is the abs-and-track-best comparator pair instead of a triplet-emit + Vec-push. Rust wins at **1.55×** over Kāra seq — a meaningful gap, not the dead-heat of kata 15 (where the per-iter Vec output dominated and both monomorphizing compilers landed identically). The gap shrinks once you account for the inner body: with no per-call nested-Vec allocation, the hot path here is `s[i] + s[lo] + s[hi]`, two `abs(sum - target)` calls, and a couple of integer compares. Rust's `(sum - target).abs()` lowers to a branchless absolute-value sequence; Kāra's `abs_i64` is a hand-rolled `if x < 0 { -x } else { x }`, and the seq-lane numbers suggest the LLVM IR Kāra emits for the helper isn't getting the same branchless lowering Rust's intrinsic does. **This is a legitimate compiler-side perf gap surfaced by the kata** (memory: katas are bug-finders) — flagged for follow-up in karac, not papered over here.

Kāra still beats C on this kata: **C trails Kāra seq by 1.17×** (113.5 vs 96.8 ms). Same reason as kata 15 — `qsort`'s indirect comparator call lands per comparison, while Kāra and Rust monomorphize their sorts. The lighter body actually *amplifies* the qsort overhead's relative contribution, which is why C drops from kata 15's 1.14× behind Kāra to 1.17× behind here. Go trails further at **1.91× behind Kāra seq**: `sort.Slice` also pays the per-comparison closure indirect call, with GC and scheduler overhead on top.
...
Same rationale as [`1-two-sum/README.md § Why this kata is in the harness`](../1-two-sum/README.md#why-this-kata-is-in-the-harness): Rust is Kāra's semantic peer (compiled, ownership-aware), so the headline ratio is the codegen-vs-Rust gap. C calibrates the LLVM-backend floor, Go is the cross-runtime data point, and Python is the ergonomic foil. On this kata the **Kāra/Rust 1.55× gap is the load-bearing result** — kata 15 was a tie because the per-iter Vec output dominated; this kata removes that output, exposes the inner-body codegen, and surfaces a real `abs_i64`-lowering gap to chase in karac. Kāra still leads C (qsort indirect-call penalty) and Go (sort.Slice closure + GC).
