...
// Rust mirror of bench/scramble_string.kara. Each iteration builds a length-12 string
// and a coprime-step permutation of it, runs the O(n⁴) top-down memoized scramble, and
// folds the filled memo state into a work-sensitive checksum added to an associative
// sum. Same len/K. Compiled with `rustc -O`. The rayon/ sibling parallelises the same
// reduction. See ../README.md § Benchmarks.
