#!/usr/bin/env bash
# Wall-clock comparison across implementations of LeetCode #25 (iterative).
# See ../README.md § Benchmarks for what these numbers mean.
#
# Seq lane (BENCH.md two-lane discipline): the loop builds one fresh
# 100-node list, reverses it in groups of k = 5 in place, and sums it,
...
# before timing. Each iter sums the reversed list — the reversal re-links every
# node but moves no values, so the sum is the invariant N·(N+1)/2 = 5050;
# K=500_000 iters → 5050 * 500000 = 2_525_000_000. A reversal that drops or
...
bench_begin id=25 slug=reverse-nodes-in-k-group group=1-100 \
    title="Reverse Nodes in k-Group" \
    workload="iterative: build 100-node list, reverse in groups of k=5 in place, sum, K=500_000 (py K=100k)" \
    sink="$expected"
...
ovf_rt_cmds "${STEM}" "iterative" seq
...
ce_cmd --lang kara --approach iterative --mode codegen \
    ...
    --cmd "sh -c \"KARAC_AUTO_PAR=0 karac build ${STEM}.kara >/dev/null && mv ${STEM} target/${STEM}_kara.ce\""
