...
Snapshot — M5 Pro, 2026-05-23, hyperfine `--warmup 5 --runs 30 --shell=none`, native binaries via `karac build`, `rustc -O`, `clang -O3`, `go build`:

| Implementation | Wall time | User-CPU | Within-workload ratio |
|---|---|---|---|
| c    iterative (clang -O3)       | **400.5 ms ± 15.3 ms** | 394.3 ms | 0.79× of Kāra |
| go   iterative                   | 449.2 ms ± 6.6 ms      | 463.3 ms | 0.88× of Kāra |
| **kāra iterative (codegen)**     | **507.6 ms ± 8.2 ms**  | 501.3 ms | **1.00×** (baseline) |
| rust iterative (`Rc<RefCell>`)   | 815.3 ms ± 11.1 ms     | 808.4 ms | 1.61× of Kāra |

**Auto-par observation.** `nm -gU target/iterative_kara | grep karac_par_run` shows the binary picks up an implicit par-group activation on the prologue's `let b = make_nines(n); let l1 = from_array(a.as_slice());` pair (per BENCH.md § Implicit auto-par detection). Per-branch work is ~10 μs each (one Vec push-loop + one linked-list build at n=100); `karac_par_run`'s ~7 ms dispatch overhead is well above that, so the dispatch does not deliver a wall-time win. Binary is 311.9 KiB vs 49.0 KiB with `KARAC_AUTO_PAR=0` (+263 KiB par-machinery), and the wall-time difference is within run-to-run noise (~7 ms — visible but well below σ on the 500 ms range). This is the same wasteful-activation pattern kata [#91](../91-decode-ways/) hit; the karac cost-model fix to close it lives at the codegen layer (not at this kata's source). Tracked as a karac follow-up — per BENCH.md the kata's bench.sh stays single-binary on the default build until the karac fix lands.

Allocator-bound + RC-discipline-bound shape: a hot inner loop walking and growing a chain of ~100 small heap nodes, then dropping the chain when the result goes out of scope. **Kāra is 1.61× faster than Rust's `Rc<RefCell<>>`** and sits between Go and C on this lane. The win over Rust comes from two karac codegen optimizations specifically targeting `Option[shared T]` linked-list shapes:
...
| kāra iterative (codegen) | 1.6 MiB |
| py   iterative | 9.5 MiB |
| go   iterative | 9.8 MiB |

Peak RSS is bounded and comparable to Rust's; the per-iteration allocate-then-drop discipline keeps one ~100-node chain live at a time. Kāra's 0.4 MiB headroom over Rust is the par-pool initialization paid by the linked-but-not-actually-dispatched `karac_par_run` symbol (see *Auto-par observation* above) — would drop to Rust parity once the karac cost-model fix lands. Go's baseline includes the runtime + GC; Python's includes the CPython interpreter.
