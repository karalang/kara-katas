#!/usr/bin/env bash
# Wall-clock comparison across implementations of LeetCode #13.
# See ../README.md § Benchmarks for what these numbers mean.
#
# Two-lane kata (BENCH.md § Implicit auto-par): karac's auto-par-on-
# reduction recognizes the `sum = sum + roman_to_int(r)` accumulator in
...
bench_begin id=13 slug=roman-to-integer group=1-100 \
    title="Roman to Integer" \
    workload="kara/rust/c/go K=10^7 reduction over roman-to-int; py K=10^6 scaled-down" \
    sink="$expected"
