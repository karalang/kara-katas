...
// C mirror of bench/greedy.kara. Same K, LCG fill, sink formula — see
...
int main(void) {
    ...
    for (int64_t k = 0; k < k_iters; k++) {
        ...
        // .kara and .rs versions. Free at end of iter mirrors the
        // per-iter drop of the Vec[char].
        int32_t *r = (int32_t *)malloc(15 * sizeof(int32_t));
        int len = int_to_roman(num, r);
        sum += roman_to_int(r, len);
        free(r);
    }
    printf("%lld\n", (long long)sum);
    return 0;
