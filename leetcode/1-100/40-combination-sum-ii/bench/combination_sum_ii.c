// LeetCode #40 bench mirror — C, the sorted same-level-dedup backtracker (★).
//
// Mirrors bench/combination_sum_ii.kara: sort once, an index-bounded DFS recursing at i+1
// (each element used once) with the same-level duplicate skip and the suffix break,
// snapshotting the path into a growable result of {data,len} combinations at each leaf —
// the C analog of Vec[Vec[i64]] + path.clone(). Same workload (TOTAL enumerations over a
// fixed duplicate-heavy multiset, target = 10 + k%13) and the same position-weighted
// checksum as every other mirror.
...
static void backtrack(const int64_t *candidates, size_t n, size_t start,
    ...
    for (size_t i = start; i < n; i++) {
        ...
        if (i > start && c == candidates[i - 1]) continue;
        vec_push(path, c);
        backtrack(candidates, n, i + 1, remaining - c, path, out);
        vec_pop(path);
    }
}
...
int main(void) {
    ...
    int64_t candidates[10] = {1, 1, 2, 2, 3, 3, 4, 5, 6, 7};
    qsort(candidates, 10, sizeof(int64_t), cmp_i64);

    int64_t acc = 0;
    for (int64_t k = 0; k < total; k++) {
        int64_t target = 10 + (k % 13);

        VecVec out;
        vecvec_init(&out);
        Vec path;
        vec_init(&path);
        backtrack(candidates, 10, 0, target, &path, &out);
        free(path.data);
