...
|---|---|---|
| c    recover (clang -O3)                            | 271.0 ± 4.2 ms | raw `malloc` pointer |
| go   recover (GC)                                   | 505.7 ± 21.8 ms | GC `*Node` |
| **kāra recover**                                    | **663.1 ± 37.9 ms** | **`shared` (RC)** |
| rust recover (rustc -O)                             | 749.3 ± 23.6 ms | `Rc<RefCell>` |
| rust recover (rustc -O, overflow-checks=on)          | 758.2 ± 20.2 ms | `Rc<RefCell>` |

**Kāra beats its closest semantic peer.** The honest match for Kāra's `shared struct TreeNode` — reference-counted *and* interior-mutable — is Rust's `Rc<RefCell<Node>>`, and against it Kāra is **1.13× faster** (663 vs 749 ms): collecting node handles bumps a refcount on every `clone`, and `Rc<RefCell>` layers a runtime borrow-flag check on top of that count on every `borrow()`/`borrow_mut()`, which Kāra's leaner RC handle avoids. The gap runs the other way against the representations that carry *no* per-access ownership bookkeeping: **C's raw `malloc` pointer tree** (271 ms, 2.45×) is the metal floor with zero refcount, and **Go's GC `*Node`** (506 ms, 1.31×) chases pointers with the count deferred to the collector. So the ordering is exactly by per-node-access overhead — raw pointer < GC pointer < **Kāra RC** < `Rc<RefCell>` — and Kāra's RC lands where it should: heavier than a bare pointer, lighter than the double-counted Rust idiom that shares its safety guarantees. This is the RC-header-tax regime flagged for the tree siblings ([#92](../92-reverse-linked-list-ii/), [#95](../95-unique-binary-search-trees-ii/), [#98](../98-validate-binary-search-tree/)), here landing kāra ahead of Rust because the workload's shared-mutable-node shape forces Rust into `Rc<RefCell>` rather than a bare `Box`. Python is listed at **496 ms but ran K = 30,000 — 1/23 of the compiled iterations** (pure-Python recursion is ~23× slower per rep; timed separately, not cross-checked), so its wall-clock is not comparable.

**Flagged for the M5 re-bench** — RC-vs-GC-vs-pointer gaps have shifted on the M5's allocator in sibling katas (e.g. #98's RC gap closed, #94's Go flipped), so treat this container ordering as a data point.
