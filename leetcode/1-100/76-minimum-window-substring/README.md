...
**Kāra is last here** — 1.32× behind Go, 1.24× behind the C floor, and 1.12× behind **equal-safety** `rustc -O -C overflow-checks=on` (312.3 ms). Since that peer is *also* overflow-checked, the 1.12× is a **pure codegen gap**, not a safety tax: kāra retires **12.46 G** instructions to C's 7.58 G (1.64×) and rust_ovf's 11.14 G (1.12×) at the *highest* IPC of the three (8.00) — so it is more, cheap instructions, not slower ones. The cause is **diffuse** (~4-5 extra instructions per outer iteration), and an IR-level investigation (`B-2026-07-10-5`) has already *ruled out* the two tempting explanations: the surviving `s[l]` bounds check is **soundly required** (its `l < n` bound holds only through the algorithm's `formed == required` invariant, which the compiler — and Rust, which checks `s[l]` identically — cannot prove), and the `Vec[i64]`-vs-`[i64;4]` count-table difference is **erased by the optimizer** (rebuilding kāra's tables as fixed arrays moved instructions 0.03%). It is a genuine, small, not-yet-localized codegen gap — tracked honestly rather than hand-waved. The M5 narrowed it from the container's 1.55× vs C to 1.24×, but — unlike [#73](../73-set-matrix-zeroes/)/[#74](../74-search-a-2d-matrix/) — did not reorder it. Python at 1/10 the iteration count is ~1.78 s (~50× kāra projected).
...
Same rationale as [`1-two-sum/README.md § Why this kata is in the harness`](../1-two-sum/README.md#why-this-kata-is-in-the-harness): Rust is Kāra's semantic peer, so the headline seq ratio is the codegen-vs-Rust gap — and here it is an honest **loss**: kāra trails equal-safety `rustc -C overflow-checks=on` by 1.12× on the M5, a small, diffuse, not-yet-localized codegen gap (`B-2026-07-10-5`; the two obvious explanations were investigated at the IR level and disproven). That is exactly the value of keeping Rust in the harness — it turns a soft "kāra is fast" into a precise, tracked gap. C calibrates the metal floor (kāra 1.24× behind it), Go is the other native data point (fastest here), Python (run at 1/10 the iteration count, timed separately) the ergonomic foil.
