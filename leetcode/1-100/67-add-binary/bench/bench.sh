...
bench_begin id=67 slug=add-binary group=1-100 \
    title="Add Binary" \
    workload="column-add carry-ripple of two 96-bit windows x 3.6M pairs over a 2M-bit buffer (build-once + punch); sink=popcount total" sink="$expected"
