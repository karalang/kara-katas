...
bench_begin id=9 slug=palindrome-number group=1-100 \
    title="Palindrome Number" workload="K=50M palindrome reduction" \
    sink="$expected"
...
echo
echo "=== binary size ==="
size_put --lang kara --approach "$STEM" --lane seq --mode codegen --path "target/${STEM}_kara_seq"
...
mem_put --lang kara --approach "$STEM" --lane seq --mode codegen --bytes "$(mem_peak ./target/${STEM}_kara_seq)"
...
mem_put --lang go   --approach "$STEM" --lane par --mode native  --bytes "$(mem_peak ./target/${STEM}_go_par)"
