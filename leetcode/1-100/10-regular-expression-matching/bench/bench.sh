#!/usr/bin/env bash
# Wall-clock comparison across implementations of LeetCode #10.
...
# accumulator in main's K=10M loop and emits a `karac_par_reduce`
# dispatch by default. We build TWO kara binaries:
#
#   regex_kara_seq — KARAC_AUTO_PAR=0, single-threaded.
#                    Within-lane comparator vs rust / c / go.
#   regex_kara     — default karac build, auto-par on.
...
bench_begin id=10 slug=regular-expression-matching group=1-100 \
    title="Regular Expression Matching" \
    workload="kara/rust/c/go K=10^7 reduction over regex matches; py K=10^6 scaled-down" \
    sink="$expected"
...
rt_cmd --lang kara --approach regex --lane seq --mode codegen \
    --name "kara ${STEM} (seq, KARAC_AUTO_PAR=0)" --cmd "./target/${STEM}_kara_seq"
rt_cmd --lang rust --approach regex --lane seq --mode native \
    --name "rust ${STEM}" --cmd "./target/${STEM}"
ovf_rt_cmds "${STEM}" "regex" seq
rt_cmd --lang c --approach regex --lane seq --mode native \
...
rt_cmd --lang kara --approach regex --lane par --mode codegen \
    --name "kara ${STEM} (auto-par default)" --cmd "./target/${STEM}_kara"
rt_cmd --lang c --approach regex --lane par --mode native \
    --name "c    ${STEM} (pthreads — metal floor)" --cmd "./target/${STEM}_c_par"
rt_cmd --lang rust --approach regex --lane par --mode native \
    --name "rust ${STEM} (rayon par_iter)" --cmd "./target/${STEM}_rayon"
rt_cmd --lang go --approach regex --lane par --mode native \
...
rt_cmd --lang python --approach regex --lane seq --mode interp \
...
ce_cmd --lang kara --approach regex --mode codegen \
...
ce_cmd --lang rust --approach regex --mode native \
    --prepare "rm -f target/${STEM}" \
    --name "rustc -O ${STEM}.rs" --cmd "rustc -O ${STEM}.rs -o target/${STEM}"
ce_cmd --lang c --approach regex --mode native \
...
echo
echo "=== binary size ==="
size_put --lang kara --approach regex --lane seq --mode codegen --path "target/${STEM}_kara_seq"
size_put --lang kara --approach regex --lane par --mode codegen --path "target/${STEM}_kara"
size_put --lang c    --approach regex --lane par --mode native  --path "target/${STEM}_c_par"
size_put --lang rust --approach regex --lane par --mode native  --path "target/${STEM}_rayon"
size_put --lang go   --approach regex --lane par --mode native  --path "target/${STEM}_go_par"
size_put --lang rust --approach regex --lane seq --mode native  --path "target/${STEM}"
size_put --lang c    --approach regex --lane seq --mode native  --path "target/${STEM}_c"
size_put --lang go   --approach regex --lane seq --mode native  --path "target/${STEM}_go_seq"

echo
echo "=== runtime memory (peak) ==="
mem_put --lang kara --approach regex --lane seq --mode codegen --bytes "$(mem_peak ./target/${STEM}_kara_seq)"
mem_put --lang kara --approach regex --lane par --mode codegen --bytes "$(mem_peak ./target/${STEM}_kara)"
mem_put --lang c    --approach regex --lane par --mode native  --bytes "$(mem_peak ./target/${STEM}_c_par)"
mem_put --lang rust --approach regex --lane par --mode native  --bytes "$(mem_peak ./target/${STEM}_rayon)"
mem_put --lang go   --approach regex --lane par --mode native  --bytes "$(mem_peak ./target/${STEM}_go_par)"
mem_put --lang rust --approach regex --lane seq --mode native  --bytes "$(mem_peak ./target/${STEM})"
mem_put --lang c    --approach regex --lane seq --mode native  --bytes "$(mem_peak ./target/${STEM}_c)"
mem_put --lang go   --approach regex --lane seq --mode native  --bytes "$(mem_peak ./target/${STEM}_go_seq)"
...
for src in "${STEM}.kara"; do
    ...
    cmem_put --lang kara --approach regex --mode codegen --bytes "$bytes"
done
for src in "${STEM}.rs"; do
    out="target/$(basename "$src" .rs)"
    rm -f "$out"
    cmem_put --lang rust --approach regex --mode native --bytes "$(mem_peak rustc -O "$src" -o "$out")"
done
for src in "${STEM}.c"; do
    out="target/$(basename "$src" .c)_c"
    rm -f "$out"
    cmem_put --lang c --approach regex --mode native --bytes "$(mem_peak clang -O3 "$src" -o "$out")"
done
