...
| Run | Mean ± σ | Gap |
|---|---|---|
| c    two_pointer (clang -O3) | 10.8 ± 0.4 ms | 1.34× ahead of kāra |
| **kāra two_pointer (codegen)** | **14.5 ± 0.5 ms** | — |
| rust two_pointer | 16.4 ± 0.5 ms | **kāra 1.13× faster than Rust** |
| go   two_pointer | 19.3 ± 0.7 ms | kāra 1.33× ahead of Go |

(The 2026-05-18 snapshot read kāra 14.6 ± 0.6 / rust 16.6 ± 0.5 — both reproduce within σ, and today's kāra, Rust, and C binaries are byte-identical to the May artifacts; the kāra binary even rebuilt byte-for-byte under the newer karac. C and Go rows are benched here for the first time.)
...
| Run | Mean ± σ |
|---|---|
| `kara two_pointer` (codegen) | 14.5 ± 0.5 ms |
| `rust two_pointer` | 16.4 ± 0.5 ms |
| `py two_pointer` | 699.4 ± 10.8 ms |
...
|---|---|---|
| `karac build two_pointer.kara` | 69.4 ± 2.3 ms | 33.0 KiB |
| `rustc -O two_pointer.rs` | 89.4 ± 1.3 ms | 455.4 KiB |
| `clang -O3 two_pointer.c` | 47.6 ± 1.1 ms | 32.8 KiB |

Kāra compiles this kata **1.29× faster** than `rustc -O` and produces a binary **~93% smaller** — within 184 bytes of C's (33,752 vs 33,568 B). (The 2026-05-18 snapshot read `karac build` at 56.1 ± 0.9 ms against the karac installed at the time; the May-30 karac reinstall plus the 06-05 environment band account for today's 69.4 — the emitted binary is byte-identical across the two compilers, so codegen output is unchanged. rustc held flat 88.6 → 89.4.) The much smaller binary tracks the much narrower runtime surface this workload reaches — `Vec.filled` + indexed read/write, `Slice[i64]` indexing, `println(i64)` — and nothing else. The cross-archive LTO + DCE work landed 2026-05-12 elides the rest of the runtime (HTTP, JSON, tokio subgraph, `Map`, `String`, shared structs) cleanly when downstream features aren't reached. Same shape as kata [#4](../4-median-of-two-sorted-arrays/#compile-time-and-binary-size).
