...
`--warmup 5 --runs 30 --shell=none`. All single-threaded. **Apple M5 Pro numbers — ⚠ these PREDATE the [kara `B-2026-07-15-21`](https://github.com/karalang/kara/blob/main/docs/bug-ledger.jsonl) fix and await an M5 re-bench** (the fixed compiler roughly halves the Kāra row — see the container reference below):
