...
|---|---|
| c    same_tree (clang -O3, `*Node`)                 | 197.3 ± 8.5 ms |
| rust same_tree (rustc -O, overflow-checks=on)        | 217.6 ± 7.3 ms |
| rust same_tree (rustc -O, `Box` + borrow)           | 224.8 ± 7.2 ms |
| go   same_tree (`*Node`, GC)                        | 306.9 ± 6.7 ms |
| **kāra same_tree**                                  | **596.1 ± 21.4 ms** |

**Kāra trails on the corpus's most-exposed traversal — a read-only pointer chase with nothing to amortize.** Each rep is *only* `match` + field-access per node: no allocation, no compute, no strings, so any per-node overhead *is* the entire cost. Kāra is **3.0× behind C**, **2.65× behind Rust**, and **1.94× behind Go**. I profiled *why* rather than assume: a `ref Option[TreeNode]` borrow variant of `is_same` (matching the mirrors' `&`/pointer traversal, no per-node refcount) runs at **~530 ms vs 596** — so passing the `shared` node **by value**, which rc-inc/decs each visited node, accounts for only **~11 %** of the gap. The **dominant** remainder is Kāra's per-node **traversal codegen**: decoding the `Option[shared]` niche, matching it, and GEP-ing the shared field is heavier than C/Rust's raw null-check-and-deref, and there is no other work in the loop to hide it behind. Both are real codegen headroom — RC-retain *elision* on a non-escaping borrow, and leaner `Option[shared]` match/field lowering — flagged as such rather than waved off. This is the RC/traversal regime at its most exposed, the read-only-walk analogue of [#95](../95-unique-binary-search-trees-ii/)'s pure-allocation regime; the compute- and check-bound siblings ([#96](../96-unique-binary-search-trees/) fastest, [#99](../99-recover-binary-search-tree/) ahead of `Rc<RefCell>`) bracket it from the other side. Python is listed at **475 ms but ran K = 300,000 — 1/20 of the compiled iterations** (timed separately, not cross-checked), so its wall-clock is not comparable.

**Flagged for the M5 re-bench** — the per-node-overhead gap should carry, but the corpus has seen container tree-traversal orderings compress sharply on the M5's wider core (see [#97](../97-interleaving-string/)), so treat the *margin* as a data point, not a verdict.

> **Binary size caveat.** As with the sibling katas, the kāra binary in `results.container-x86.json` measures large (≈ 332 KB) — a build-linkage artifact of this container run (heap-growing programs retain the backtrace-symbolizer tree `--gc-sections` should strip; a correct build is ~15 KB), independent of the runtime numbers, flagged for the M5.
...
Same rationale as [`1-two-sum/README.md § Why this kata is in the harness`](../1-two-sum/README.md#why-this-kata-is-in-the-harness): Rust is Kāra's semantic peer, so the headline ratio is the codegen-vs-Rust gap — here at matched read-only tree traversal (`shared` handle walk vs `Box` + `&`-borrow), where the profiled RC-retain-vs-borrow split (~11 %) and the per-node lowering gap (the rest) are isolated. C's raw pointer chase is the metal floor, Go the GC data point, Python (run at a fraction of the iteration count, timed separately, not cross-checked) the ergonomic foil.
