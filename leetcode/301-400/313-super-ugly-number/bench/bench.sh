#!/usr/bin/env bash
# Wall-clock comparison across implementations of LeetCode #313.
...
bench_begin id=313 slug=super-ugly-number group=301-400 \
    title="Super Ugly Number" \
    workload="allocate the 100000-term sequence array and 40 stream pointers once, then punch 30 complete k-way-merge generations through them, each swapping one prime at an index drawn from the running checksum so no generation can be hoisted; sink=masked running total" sink="$expected"
