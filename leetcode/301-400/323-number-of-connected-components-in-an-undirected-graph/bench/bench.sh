#!/usr/bin/env bash
# Wall-clock comparison across implementations of LeetCode #323.
...
bench_begin id=323 slug=number-of-connected-components-in-an-undirected-graph group=301-400 \
    title="Number of Connected Components in an Undirected Graph" \
    workload="16 passes over one disjoint-set forest of 1000000 nodes, each resetting it to singletons and feeding it M random LCG edges, M cycling through 250000, 500000, 750000 and 1000000 (below to above the giant-component threshold), every edge one merge (two path-compressing finds and a union by size); sink=rolling hash of each pass's component count and a stride-9973 sample of roots" sink="$expected"
