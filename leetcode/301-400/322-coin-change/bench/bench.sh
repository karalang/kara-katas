#!/usr/bin/env bash
# Wall-clock comparison across implementations of LeetCode #322.
...
bench_begin id=322 slug=coin-change group=301-400 \
    title="Coin Change" \
    workload="24 passes, each drawing 20 distinct coins up to 3000 (two passes in three draw them on a grid of 2 or 3, and 8 of those 16 targets are unreachable) and a target in [500000, 1000000], then filling the bottom-up fewest-coins table once (one pass over every amount, trying every coin); sink=rolling hash of each answer and of a stride-9973 sample of each table, plus the count of reachable targets" sink="$expected"
