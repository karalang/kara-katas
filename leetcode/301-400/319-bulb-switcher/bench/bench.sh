#!/usr/bin/env bash
# Wall-clock comparison across implementations of LeetCode #319.
...
bench_begin id=319 slug=bulb-switcher group=301-400 \
    title="Bulb Switcher" \
    workload="10 passes, each simulating all n rounds over an n-bulb byte array for a different n from 6,000,000 down to 5,189,901; sink=masked fold of each pass's lit-bulb count and lit-index sum" sink="$expected"
