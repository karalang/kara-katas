#!/usr/bin/env bash
# Wall-clock comparison across implementations of LeetCode #310.
...
bench_begin id=310 slug=minimum-height-trees group=301-400 \
    title="Minimum Height Trees" \
    workload="build four 60000-node random trees once in flat CSR, then punch 950 leaf-peeling passes, each selecting its tree from the previous pass's answer so no pass can be hoisted; sink=masked sum of centre ids" sink="$expected"
