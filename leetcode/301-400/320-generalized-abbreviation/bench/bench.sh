#!/usr/bin/env bash
# Wall-clock comparison across implementations of LeetCode #320.
...
bench_begin id=320 slug=generalized-abbreviation group=301-400 \
    title="Generalized Abbreviation" \
    workload="8 passes, each walking all 2^20 masks of a fresh 20-letter word and run-length encoding every one into a reusable byte buffer; sink=rolling hash of every emitted byte, plus the total character count" sink="$expected"
