#!/usr/bin/env bash
# Wall-clock comparison across implementations of LeetCode #387.
...
bench_begin id=387 slug=first-unique-character-in-a-string group=301-400 \
    title="First Unique Character in a String" \
    workload="build a 4000-byte haystack once (25 repeating chars + one unique 'z' at the end), then 2000 punches; each punch mutates one byte and runs both a first-unique scan and a keys()-walk unique tally over a 26-key hash map; sink=sum of both results. NOTE: C hand-rolls an open-addressing hash map (same insert/get/key-walk semantics), NOT a direct-address table." sink="$expected"
