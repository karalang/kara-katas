// LeetCode #394 bench harness — Rust peer (rustc -O, single-thread).
//
// Same iterative-stack byte-scan decode as the Kāra mirror; `cur*count` uses
// `str::repeat` (Kāra's `String.repeat` analog). Sink = ITERS * 52 = 41600000.
...
fn main() {
    let mut sum: i64 = 0;
    for _ in 0..ITERS {
        sum += decode_string(ENCODED).len() as i64;
    }
    println!("{}", sum);
}
