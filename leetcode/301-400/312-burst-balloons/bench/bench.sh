#!/usr/bin/env bash
# Wall-clock comparison across implementations of LeetCode #312.
...
bench_begin id=312 slug=burst-balloons group=301-400 \
    title="Burst Balloons" \
    workload="build one 300-balloon row once, then punch 88 full interval-DP solves over a 302x302 table, each perturbing one balloon from the running checksum so no solve can be hoisted; sink=masked running total" sink="$expected"
