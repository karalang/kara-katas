#!/usr/bin/env bash
# Wall-clock comparison across implementations of LeetCode #306.
...
bench_begin id=306 slug=additive-number group=301-400 \
    title="Additive Number" \
    workload="build 220 candidate strings of 18 digits once as one flat digit array (every 25th a planted additive positive), then punch 90 full O(n^3) additive scans over them, each perturbing one digit at a site derived from the running checksum so no scan repeats; sink=masked running total" sink="$expected"
