#!/usr/bin/env bash
# Wall-clock comparison across implementations of LeetCode #318.
...
bench_begin id=318 slug=maximum-product-of-word-lengths group=301-400 \
    title="Maximum Product of Word Lengths" \
    workload="build a 6000-word corpus once (lengths 1-16 over a random 7-letter window each), then 15 passes, one word rewritten per pass, each pass recording every word's best disjoint partner; sink=masked fold of per-pass (sum, max)" sink="$expected"
