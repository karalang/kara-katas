/*
 * LeetCode 226 — recursive DFS invert, bench mirror in C.
 *
 * Algorithmic mirror of bench/recursive.{kara,rs,py}. N=2000 nodes, LCG
...
static TreeNode *invert(TreeNode *root) {
    ...
    TreeNode *new_left = invert(root->right);
    TreeNode *new_right = invert(root->left);
    root->left = new_left;
