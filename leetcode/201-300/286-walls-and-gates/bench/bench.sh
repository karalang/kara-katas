#!/usr/bin/env bash
# Wall-clock comparison across implementations of LeetCode #286.
...
# The cross-language SEQ table compares single-threaded against single-threaded,
# so the kara SEQ row is <stem>_seq.kara with auto-par OFF. The kara PAR row is
# <stem>.kara and is compared only against mirrors that ALSO opted into
# parallelism. A cross-lane row would measure the comparator's choice, not the
# compiler.
...
bench_begin id=286 slug=walls-and-gates group=201-300 \
    title="Walls and Gates" \
    workload="64 independent 512x512 boards, 20% walls and 1% gates, multi-source BFS on each; sink is the summed finite distance plus the unreachable-cell count" sink="$expected"
