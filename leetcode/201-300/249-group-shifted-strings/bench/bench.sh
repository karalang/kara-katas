#!/usr/bin/env bash
# Wall-clock comparison across implementations of LeetCode #249.
...
bench_begin id=249 slug=group-shifted-strings group=201-300 \
    title="Group Shifted Strings" \
    workload="build 120k shift-derived words once (length 3-12 over a small seed set), then 12 rounds of canonical-form grouping into Map[String, Vec[String]] with in-place entry().or_insert().push(); sink = group count + first-seen-order checksum" sink="$expected"
