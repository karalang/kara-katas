#!/usr/bin/env bash
# Wall-clock comparison across implementations of LeetCode #255.
...
bench_begin id=255 slug=verify-preorder-sequence-in-bst group=201-300 \
    title="Verify Preorder Sequence in BST" \
    workload="build one valid 200k-node BST preorder once, then 250 rounds of full stack verification; sink = accumulated verdict-and-bound checksum" sink="$expected"
