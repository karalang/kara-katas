#!/usr/bin/env bash
# Wall-clock comparison across implementations of LeetCode #282.
...
# The SEQ table compares single-threaded against single-threaded; the PAR row is
# compared only against mirrors that also opted into parallelism.
...
bench_begin id=282 slug=expression-add-operators group=201-300 \
    title="Expression Add Operators" \
    workload="220 nine-digit inputs searched exhaustively for expressions hitting a target; sink = solution count plus a hash of their lengths" sink="$expected"

echo "=== runtime — short workloads (compiled) ==="
rt_begin --warmup 5 --runs 30
# PAR lane — auto-par kara against mirrors that also reached for threads.
