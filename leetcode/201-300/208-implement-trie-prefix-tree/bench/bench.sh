...
bench_begin id=208 slug=implement-trie-prefix-tree group=201-300 \
    title="Implement Trie (Prefix Tree)" \
    workload="build a 30k-word index-pool trie once, then walk 8M PRNG query words (data-dependent pointer-chase; sink = weighted prefix+exact hits)" sink="$expected"
