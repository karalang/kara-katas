#!/usr/bin/env bash
# Wall-clock comparison across implementations of LeetCode #251.
...
bench_begin id=251 slug=flatten-2d-vector group=201-300 \
    title="Flatten 2D Vector" \
    workload="build one ragged 2D input once (20k rows, ~45% empty, ~50k elements), then 1500 full drains through the lazy (row,col) iterator; sink = positional checksum of every yielded element" sink="$expected"
