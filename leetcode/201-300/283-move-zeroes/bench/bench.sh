#!/usr/bin/env bash
# Wall-clock comparison across implementations of LeetCode #283.
...
bench_begin id=283 slug=move-zeroes group=201-300 \
    title="Move Zeroes" \
    workload="60 rounds of the write-cursor pass over a 2M-element array that is ~50% zeros, refreshed each round; sink = position-weighted hash plus the write count" sink="$expected"
