#!/usr/bin/env bash
# Wall-clock comparison across implementations of LeetCode #246.
...
bench_begin id=246 slug=strobogrammatic-number group=201-300 \
    title="Strobogrammatic Number" \
    workload="build 20,000 length-32 numbers once (constructed strobogrammatic, 1 in 8 corrupted at a random position so rejects happen mid-scan rather than on the first character), then 100 passes of the two-pointer check over all of them = 2,000,000 calls; sink is a rolling polynomial hash of the verdicts" sink="$expected"
