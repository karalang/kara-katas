...
**Resolved 2026-08-15 — the residual deficit to `clang -O3` is ENTIRELY loop unrolling, and it is a `wontfix`.** Filed and pinned the same day as kara **`B-2026-08-15-31`**. Dynamic instruction counts (retired, via `scripts/pmc.c`) close the question the paragraphs below leave open: kara 527.4M instrs against `clang -O3`'s 370.7M, but `clang -O3 -fno-unroll-loops` lands at 500.6M — 1.05× of kara — and kara is 11% *ahead* of `clang -O2` on cycles. The per-pair sequences are instruction-for-instruction identical; kara's 5 instructions of loop bookkeeping are what clang's unroll amortizes to zero. Three ways of making karac's own unroll machinery fire on this loop were built and measured, and each either misfires (a converging loop's trip count has no compile-time bound — kata #189's million-iteration reverse took `karac build` from 0.6 s to killed at 300 s) or is a corpus net regression. Read the ledger row before reopening this subject.
