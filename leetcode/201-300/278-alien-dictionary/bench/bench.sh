#!/usr/bin/env bash
# Wall-clock comparison across implementations of LeetCode #278.
...
# SEQ twin (auto-par OFF). The par lane does not currently fan out — see
# ../README.md § Benchmarks and B-2026-08-17-14 — so it is reported as a
# Kara-to-Kara pair only, with no hand-threaded mirrors to compare against yet.
...
bench_begin id=278 slug=alien-dictionary group=201-300 \
    title="Alien Dictionary" \
    workload="build one 250k-word dictionary over a planted 6-letter alphabet, then recover the alphabet 48 times; sink = order-invariant sum of (instance*1000003 + answer length)" sink="$expected"
