#!/usr/bin/env bash
# Wall-clock comparison across implementations of LeetCode #259.
...
bench_begin id=259 slug=3sum-smaller group=201-300 \
    title="3Sum Smaller" \
    workload="build one 4000-element array once, then 26 rounds of sorted two-pointer triplet counting with an in-band target; sink = accumulated counts" sink="$expected"
