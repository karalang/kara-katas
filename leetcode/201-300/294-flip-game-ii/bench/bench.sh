#!/usr/bin/env bash
# Wall-clock comparison across implementations of LeetCode #294.
...
bench_begin id=294 slug=flip-game-ii group=201-300 \
    title="Flip Game II" \
    workload="900 random boards of length 22 across three '+' densities (15/50/85%), each solved by memoized backtracking with a fresh Map[String,bool]; sink is the first-player win count plus a checksum over each memo's final size" sink="$expected"
