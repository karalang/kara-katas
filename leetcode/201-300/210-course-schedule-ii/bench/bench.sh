...
bench_begin id=210 slug=course-schedule-ii group=201-300 \
    title="Course Schedule II" \
    workload="build a 20k-node/80k-edge random DAG once (CSR); run Kahn topo-sort 800 times, punching one blocked course per pass; sink = sum of emitted count + order checksum" sink="$expected"
