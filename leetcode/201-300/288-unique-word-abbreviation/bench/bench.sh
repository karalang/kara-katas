#!/usr/bin/env bash
# Wall-clock comparison across implementations of LeetCode #288.
# See ../README.md § Benchmarks for what these numbers mean.
#
# Generated by scripts/new-bench.sh — keep in sync via that script.
# Seq + par lanes. Requires: hyperfine, rustc, clang, go, karac.
...
# --- PAR twins -------------------------------------------------------------
# UNLIKE the other par-lane katas, there is no `<stem>_seq.kara` here and no
# `#[par_order_free]` in the source. This loop is picked up by the automatic
# `parallel_reduction` path unaided, so the two Kara rows are the SAME FILE
# built twice and the only difference is the compiler's own decision. Writing a
# byte-identical `_seq.kara` to satisfy the usual convention would just be a
# duplicate file asserting nothing.
#
# The PAR build must NOT set KARAC_AUTO_PAR=0 — that variable is the
# auto-parallelizer's kill switch, and setting it here would silently produce a
# sequential binary and a par row measuring the seq lane.
if [ ! -x "target/${STEM}_par_kara" ] || [ "${STEM}.kara" -nt "target/${STEM}_par_kara" ] || [ "$(command -v karac)" -nt "target/${STEM}_par_kara" ]; then
    echo "compiling ${STEM}.kara (auto-par ON) ..." >&2
    karac build "${STEM}.kara" -o "target/${STEM}_par_kara" >/dev/null
fi
if [ ! -x "target/${STEM}_seq_kara" ] || [ "${STEM}.kara" -nt "target/${STEM}_seq_kara" ] || [ "$(command -v karac)" -nt "target/${STEM}_seq_kara" ]; then
    echo "compiling ${STEM}.kara (KARAC_AUTO_PAR=0, seq twin) ..." >&2
    KARAC_AUTO_PAR=0 karac build "${STEM}.kara" -o "target/${STEM}_seq_kara" >/dev/null
fi
...
for pair in \
    ...
    "kara_par:./target/${STEM}_par_kara" \
...
bench_begin id=288 slug=unique-word-abbreviation group=201-300 \
    title="Unique Word Abbreviation" \
    workload="build a 3000-word dictionary index once, then 1M is_unique punches over a 20k-word pool (half drawn from the dictionary, half fresh); sink=count of unique answers" sink="$expected"

echo "=== runtime — short workloads (compiled) ==="
rt_begin --warmup 5 --runs 30
# PAR lane — the compiler's own fan-out against code written by someone who
# reached for threads. Kept apart from the SEQ rows deliberately: a cross-lane
# row measures the comparator's choice, not the compiler.
rt_cmd --lang kara --approach "$STEM" --lane par --mode codegen \
    --name "kara ${STEM} (codegen, auto-par)" --cmd "./target/${STEM}_par_kara"
...
# SEQ lane — single-threaded against single-threaded. The kara row is the SAME
# source as the par row, built with the auto-parallelizer switched off.
