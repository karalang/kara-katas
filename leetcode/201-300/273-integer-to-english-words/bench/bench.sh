#!/usr/bin/env bash
# Wall-clock comparison across implementations of LeetCode #273.
...
# there is no single-file rustc path for that. `cargo build --release` is
# incremental, so repeat runs are near-instant.
echo "building rayon variant (cargo) ..." >&2
( cd rayon && cargo build --release --quiet )
cp -f rayon/target/release/spell_rayon "target/${STEM}_rayon"
...
bench_begin id=273 slug=integer-to-english-words group=201-300 \
    title="Integer to English Words" \
    workload="build 200,000 integers once, then spell all of them with the star three-digit chunker 5 times; sink = sum of per-spelling hashes, order-invariant so the parallel lane can share it" sink="$expected"
...
# SEQ lane — the single-threaded per-core comparison, unchanged in shape.
...
rt_cmd --lang go --approach "$STEM" --lane seq --mode native \
    --name "go   ${STEM} (single-threaded)" --cmd "./target/go_seq"
