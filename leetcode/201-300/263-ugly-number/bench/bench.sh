#!/usr/bin/env bash
# Wall-clock comparison across implementations of LeetCode #263.
...
bench_begin id=263 slug=ugly-number group=201-300 \
    title="Ugly Number" \
    workload="sweep 10,000,000 wide LCG-drawn i64 values through the gcd-peeling ugly test, with every 512th value replaced by a constructed ugly number; sink = ugly count folded with a digest" sink="$expected"
