#!/usr/bin/env bash
# Wall-clock comparison across implementations of LeetCode #123.
...
bench_begin id=123 slug=best-time-to-buy-and-sell-stock-iii group=101-200 \
    title="Best Time to Buy and Sell Stock III" \
    workload="at-most-two-transactions DP: relax buy1/sell1/buy2/sell2 across one pass, answer sell2; N=2,000,000 LCG prices in [1,4096], K=10 SERIAL outer calls (each perturbs the opening price by the previous result, keeping the bench single-threaded like the mirrors), sum-of-results sink" sink="$expected"
