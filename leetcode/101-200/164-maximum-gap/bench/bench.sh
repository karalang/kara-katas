#!/usr/bin/env bash
# Wall-clock comparison across implementations of LeetCode #164.
...
bench_begin id=164 slug=maximum-gap group=101-200 \
    title="Maximum Gap" \
    workload="pigeonhole bucket max-gap, N=1M K=30" sink="$expected"
...
# Python is ~17x slower here (bucket allocation is pathological under CPython),
# so fewer runs keep the suite tractable; python is a scale reference only.
