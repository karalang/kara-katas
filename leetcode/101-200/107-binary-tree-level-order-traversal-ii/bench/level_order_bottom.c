// Benchmark workload for LeetCode #107 — bottom-up level order, C mirror (malloc *Node).
// Build 8 BSTs once, then K reps of DFS-with-depth bottom-up level order on a data-dependent-
// selected tree, building a fresh output (depth-indexed rows, emitted deepest-first) and
// folding level count + each level's size + every value into a rolling hash.
#include <stdio.h>
#include <stdlib.h>
...
int main(void) {
    ...
    for (long long rep = 0; rep < 1000000; rep++) {
        ...
        Rows rs = { NULL, 0, 0 };
        dfs(pool[idx], 0, &rs);
        // Build the bottom-up output: rows deepest-first (mirrors the kara `out` allocation).
        Rows out = { NULL, 0, 0 };
        for (long long d = rs.len - 1; d >= 0; d--) {
            Row* r = &rs.rows[d];
            rows_push_row(&out);
            for (long long i = 0; i < r->len; i++) row_push(&out.rows[out.len - 1], r->data[i]);
            free(r->data);
        }
        free(rs.rows);
