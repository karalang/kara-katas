#!/usr/bin/env bash
# Wall-clock comparison across implementations of LeetCode #148.
...
bench_begin id=148 slug=sort-list group=101-200 \
    title="Sort List" \
    workload="top-down merge-sort an index-pool linked list (N nodes) x passes, position-weighted checksum sink" sink="$expected"
