...
bench_begin id=165 slug=compare-version-numbers group=101-200 \
    title="Compare Version Numbers" \
    workload="build-once pool of 4096 PRNG version strings + 10M PRNG-paired compare_version calls (byte-scan parse into revision lists + element-wise compare, per-call alloc, non-vectorizing); sink = sum of (-1/0/1 result +1)" sink="$expected"
