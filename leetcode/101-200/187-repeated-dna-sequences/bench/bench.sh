...
bench_begin id=187 slug=repeated-dna-sequences group=101-200 \
    title="Repeated DNA Sequences" \
    workload="length-10 dup-window scan over a 1M-base PRNG DNA buffer x 30 punched passes; rolling 2-bit hash + direct-address 2^20 stamp/count table (C-style, uniform across all five langs)" sink="$expected"
