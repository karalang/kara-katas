...
bench_begin id=159 slug=longest-substring-with-at-most-two-distinct-characters group=101-200 \
    title="Longest Substring with At Most Two Distinct Characters" \
    workload="two-pointer sliding window over a 20000-char buffer (alphabet 8), width-96 sub-ranges x 100 punched reps; per-call fixed count table (kata Map[char,i64] as a flat array, C hand-rolls the same); data-dependent left-shrink loop" sink="$expected"
