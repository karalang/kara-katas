...
bench_begin id=167 slug=two-sum-ii-input-array-is-sorted group=101-200 \
    title="Two Sum II - Input Array Is Sorted" \
    workload="build-once 20K-elem sorted PRNG array + 20K two-pointer two-sum sweeps for guaranteed-solution PRNG targets (loop-carried lo/hi convergence, data-dependent branch, non-vectorizing); sink = sum of 1-based index pairs" sink="$expected"
