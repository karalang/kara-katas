...
bench_begin id=170 slug=two-sum-iii-data-structure-design group=101-200 \
    title="Two Sum III - Data Structure Design" \
    workload="build a 170-add sparse multiset over [0,6K) keys (~168 distinct), then 1.2M full-scan find(target) queries; sink=count of trues. NOTE: Kara/Rust/Go/Python use a hash map; C hand-rolls a direct-address count table + distinct-key list (same membership semantics)." sink="$expected"
