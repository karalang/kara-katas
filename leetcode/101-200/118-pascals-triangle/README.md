...
> **Machine.** The canonical numbers are the **Apple M5 Pro** run ([`bench/results.json`](bench/results.json), 2026-07-28), in the box below the workload description; the shared **x86-64 Linux cloud-container** run ([`bench/results.container-x86.json`](bench/results.container-x86.json)) is retained as a corroborating second host; absolute times/sizes/RSS are **not** comparable across the two hosts, only within-file cross-language ratios are the signal. Re-run `bench.sh` on the M5 to refresh the canonical table.
