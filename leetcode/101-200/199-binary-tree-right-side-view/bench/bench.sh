...
bench_begin id=199 slug=binary-tree-right-side-view group=101-200 \
    title="Binary Tree Right Side View" \
    workload="level-order right-side-view BFS over an 8191-node complete index-pool tree x 40000 punched passes (per-level allocation churn + pointer-chasing; one node value flipped per pass)" sink="$expected"
