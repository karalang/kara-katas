...
bench_begin id=162 slug=find-peak-element group=101-200 \
    title="Find Peak Element" \
    workload="build-once 4M PRNG array + 1M sliding-window slope-binary-search passes, each punched (branch-bound, non-vectorizing kernel); sink = sum of peak indices" sink="$expected"
