...
Dogfood-first kata (small BFS workload; LeetCode caps `wordList` at 5000). It was originally shipped without a cross-language benchmark for that reason — it since gained one, on a scaled build-once + punch workload; see [§ Benchmarks](#benchmarks) below. It shares the [#126](../126-word-ladder-ii/) BFS/`Map`/`String` machinery and confirms that path is leak-clean after `B-2026-07-18-52`.

## Benchmarks
<!-- bench-staleness -->
> **Figures in this section are a 2026-07-26 snapshot; the feed was last measured 2026-08-04.** Where the two disagree, [`bench/results.json`](bench/results.json) and the [charts](../../../BENCHMARKS.md) are current; the numbers below are kept because the analysis around them explains *why* the shape is what it is, and that reasoning outlives the milliseconds.
...
> **Host:** the tables in this section were measured on a shared **x86-64 Linux
> cloud container**, committed as `bench/results.container-x86.json`. Only the
> **within-file cross-language ratios** are comparable.
>
> **This kata now also has a canonical Apple M5 Pro run** (`bench/results.json`,
> 4 Aug 2026, karac `9b43809a`), so it is in the consolidated feed and the
> top-level charts — the earlier note here saying it had no M5 run and was
> deliberately absent is superseded. On that host: c 44.4 ms, go 83.9 ms, kāra
> 115.8 ms, rust 120.8 ms, rust equal-safety 120.0 ms.
>
> **The headline claim below — "Kāra is last" — does not hold on M5.** There
> Kāra and both Rust lanes are a statistical tie (1.04×, inside the ~1.15×
> noise floor), rather than Kāra trailing Rust by 1.38× as it does on the
> container. C and Go stay ahead of Kāra on both hosts, in that order. Part of
> the shift is Kāra itself: this kata's M5 row improved 1.24× (143.5 → 115.8 ms)
> between the 27 Jul 2026 and 4 Aug 2026 measurements with the source unchanged
> and identical codegen — an unattributed compiler/runtime move in the window
> spanning 28–30 Jul 2026, the same window that regressed
> [#170](../170-two-sum-iii-data-structure-design/); the open ledger row is
> cited from that kata's Notes.
> Take the standing cross-language result from the M5 feed; read the analysis
> below for the *mechanism*, which is what still holds.
