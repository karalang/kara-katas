...
**Kāra trails Rust here — and the cost is nested-`Vec[Vec]` *construction*, not the DP indexing.** Kāra runs **1.63× behind the equal-safety Rust row** (264 ms) and **2.11× behind C** (203 ms), just **ahead of Go** (454 ms). A profiling decomposition (build-only vs build+fill, flat vs nested) pins it precisely — and *disproves* the natural first guess of "nested inner-loop indexing":

- **A flat `Vec[i64]` table (indexed `dp[r*w+c]`) runs the same DP in 0.13 s** — **3.3× faster than the nested `Vec[Vec]` (0.43 s), and faster than even C's nested `long**` (0.20 s).** So Kāra's raw indexing + arithmetic is not the problem; the row-of-rows *structure* is.
- **Build-only timing shows the fill loop is nearly free:** constructing the nested table (one `malloc` per row + growth) accounts for essentially the whole nested time, while a flat table builds in ~0.02 s. The inner-loop nested indexing is *not* the bottleneck.
- **~1.7× of the gap is repeated `push`-growth reallocation:** each row is built with `Vec.new()` + a counted `push` loop, which reallocs ~log(n) times. Pre-sizing the rows with `Vec.with_capacity(n+1)` drops Kāra to **0.26 s — at parity with Rust's nested `Vec<Vec<i64>>`** — with no other change. The residual (0.26 s vs the flat 0.13 s) is the inherent per-row `malloc` of a nested container.

Overflow-checking is *not* a contributor (the two Rust rows are within noise). So the headroom is **allocation, not indexing**: a compiler that auto-pre-sizes a `Vec` built by a counted `push`-loop (the imperative sibling of the `collect`-tabulate recognizer) would recover the ~1.7×, and a flat backing store closes the rest — see [kara `B-2026-07-16-11`](https://github.com/karalang/kara/blob/main/docs/bug-ledger.jsonl). This contrasts with the read-only-traversal regime (#112, genuinely per-element codegen density); here the DP compute is cheap and the table build dominates. The alloc-bound siblings ([#113](../113-path-sum-ii/) / [#114](../114-flatten-binary-tree-to-linked-list/)) sit the other way (Kāra at-or-ahead of Rust) because their allocation is amortized differently. Verified correct **and** valgrind-clean (`0 errors, 0 bytes in use at exit`). Python is listed at **429 ms but ran K = 20,000 — 1/20 of the compiled iterations** (pure-Python is ~20× slower per rep; timed separately, not cross-checked), so its wall-clock is not comparable.
...
Same rationale as [`1-two-sum/README.md § Why this kata is in the harness`](../../1-100/1-two-sum/README.md#why-this-kata-is-in-the-harness): Rust is Kāra's semantic peer, so the headline ratio is the codegen-vs-Rust gap — here at a matched nested-`Vec` 2-D DP (`Vec[Vec[i64]]` vs `Vec<Vec<i64>>`), where Kāra trails on the *table construction* (per-row `push`-growth allocation, not the indexing) and pre-sizing the rows already reaches Rust parity. C's `long**` (with per-row malloc, for structural parity) is the metal floor, Go the GC data point, Python (run at a fraction of the iteration count, timed separately, not cross-checked) the ergonomic foil.
