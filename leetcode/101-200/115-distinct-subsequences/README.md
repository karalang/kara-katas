...
**Kāra trails Rust here — and the cost is the nested-`Vec[Vec]` structure's *per-row `malloc`*, not the DP indexing and not `push`-growth realloc.** Kāra runs **1.63× behind the equal-safety Rust row** (264 ms) and **2.11× behind C** (203 ms), just **ahead of Go** (454 ms). A reproduced decomposition (hyperfine, 30 runs, this x86 container) pins it — and *disproves* the natural first guess of "nested inner-loop indexing" **and** an earlier (wrong) guess that push-growth reallocation was the driver:

- **A flat `Vec[i64]` table (indexed `dp[r*w+c]`) runs the same DP in 0.235 s** — **1.81× faster than the nested `Vec[Vec]` (0.426 s)** (still a touch above C's nested `long**` at 0.203 s). So Kāra's raw indexing + arithmetic is not the problem; the row-of-rows *structure* — one `malloc` per row, `m+1` mallocs per rep — is what costs.
- **Pre-sizing the rows is a *measured no-op*.** Each row is `Vec.new()` + a counted `push` loop that reallocs ~log₂(n+1) times, but the rows are tiny (n ≤ 6, so `0→4→8`). Reserving `Vec.with_capacity(n+1)` gives **420 ms — indistinguishable from the 426 ms baseline (1.00×)**; a 60×-amplified microbench (5000-element rows) shows the realloc chain is real but worth only **~1.07×**. So the earlier "~1.7× of the gap is push-growth realloc, `with_capacity` drops Kāra to 0.26 s" claim **does not reproduce** — corrected here. The realloc chain is negligible; the per-row `malloc` *count* is the cost.

Overflow-checking is *not* a contributor (the two Rust rows are within noise). So the headroom is **the nested container's per-row allocation, not indexing and not realloc**: the only lever that actually moves it is a **flat backing store** (`Vec[i64]` indexed `dp[r*w+c]`, 1.81×) — a data-structure choice no auto-transform performs. The compiler *does* now auto-pre-size counted `push`-loops (the sequential collect-tabulate lowering, [kara `B-2026-07-16-11`](https://github.com/karalang/kara/blob/main/docs/bug-ledger.jsonl), plus the balanced-if/DP-base-case extension [`B-2026-07-16-17`](https://github.com/karalang/kara/blob/main/docs/bug-ledger.jsonl) this kata's rows exercise) — but on rows this small that pre-size is, as measured, a no-op. This contrasts with the read-only-traversal regime (#112, genuinely per-element codegen density); here the DP compute is cheap and the table build dominates. The alloc-bound siblings ([#113](../113-path-sum-ii/) / [#114](../114-flatten-binary-tree-to-linked-list/)) sit the other way (Kāra at-or-ahead of Rust) because their allocation is amortized differently. Verified correct **and** valgrind-clean (`0 errors, 0 bytes in use at exit`). Python is listed at **429 ms but ran K = 20,000 — 1/20 of the compiled iterations** (pure-Python is ~20× slower per rep; timed separately, not cross-checked), so its wall-clock is not comparable.
...
**Flagged for the M5 re-bench** — container orderings can shift on the M5's wider core (see [#97](../../1-100/97-interleaving-string/)); treat the *margin* as a data point. The direction (Kāra trailing Rust on nested-`Vec[Vec]` table construction) reflects allocation cost, not a microarchitectural quirk, so it should hold; the size may compress.
...
Same rationale as [`1-two-sum/README.md § Why this kata is in the harness`](../../1-100/1-two-sum/README.md#why-this-kata-is-in-the-harness): Rust is Kāra's semantic peer, so the headline ratio is the codegen-vs-Rust gap — here at a matched nested-`Vec` 2-D DP (`Vec[Vec[i64]]` vs `Vec<Vec<i64>>`), where Kāra trails on the *table construction* — the nested container's per-row `malloc` (not the indexing, and not `push`-growth realloc, which is a measured no-op on rows this small); a flat backing store is the only lever that reaches parity, a data-structure choice. C's `long**` (with per-row malloc, for structural parity) is the metal floor, Go the GC data point, Python (run at a fraction of the iteration count, timed separately, not cross-checked) the ergonomic foil.
