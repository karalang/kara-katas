// Benchmark workload for LeetCode #103 — construct binary tree, C mirror (malloc *Node).
...
// the allocation-bound churn the benchmark measures. Linear inorder scan (O(n^2)) for parity.
#include <stdio.h>
#include <stdlib.h>
...
static Node* build(const long long* pre, const long long* in,
                   long long plo, long long phi, long long ilo, long long ihi) {
    if (plo > phi) return NULL;
    long long rv = pre[plo];
    long long mid = find_in(in, ilo, ihi, rv);
    long long lsize = mid - ilo;
    Node* n = malloc(sizeof(Node));
    n->val = rv;
    n->left  = build(pre, in, plo + 1, plo + lsize, ilo, mid - 1);
    n->right = build(pre, in, plo + lsize + 1, phi, mid + 1, ihi);
    return n;
}
...
static void preorder_of(Node* n, long long* out, long long* k) { if (!n) return; out[(*k)++] = n->val; preorder_of(n->left, out, k); preorder_of(n->right, out, k); }
...
int main(void) {
    ...
    for (long long rep = 0; rep < 800000; rep++) {
        long long idx = acc % 8;
        Node* rebuilt = build(pres[idx], inos[idx], 0, bn - 1, 0, bn - 1);
