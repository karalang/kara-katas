...
bench_begin id=179 slug=largest-number group=101-200 \
    title="Largest Number" \
    workload="500-element PRNG array, 400 passes each rebuilt+perturbed then insertion-sorted under the concat comparator (a-b vs b-a, numeric); sink=sum of per-pass Horner checksums of the concatenation digit stream" sink="$expected"
