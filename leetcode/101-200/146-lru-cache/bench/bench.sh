...
bench_begin id=146 slug=lru-cache group=101-200 \
    title="LRU Cache" \
    workload="32M PRNG get/put ops, cap=1024 key-range=4096; index-pool DLL + key->slot map (C flat table, others hashmap), constant eviction" sink="$expected"
