#!/usr/bin/env bash
# Wall-clock comparison across implementations of LeetCode #122.
...
bench_begin id=122 slug=best-time-to-buy-and-sell-stock-ii group=101-200 \
    title="Best Time to Buy and Sell Stock II" \
    workload="greedy O(n) max profit with unlimited transactions: sum every positive consecutive gain profit += max(0, prices[i]-prices[i-1]); N=2,000,000 LCG prices in [1,4096], K=10 outer calls, sum-of-results sink" sink="$expected"
