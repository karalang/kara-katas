...
- **`shared struct TreeNode` (RC), read-only traversal** — both solvers walk the shared-node tree without mutating it; the recursion passes `Option[TreeNode]` **by value**, so each visited node carries a per-node RC retain/release. The compiler's read-only-walk RC-elision (kara `B-2026-07-15-21`) would normally drop that pair, but `check` is *excluded* here — its `is_balanced` wrapper calls it with a bare-identifier argument, which the conservative analysis treats as a move (see the benchmark note; this is the sweep's un-elided holdout). Node type mirrors kata [#104](../104-maximum-depth-of-binary-tree/) / [#100](../../1-100/100-same-tree/).
...
|---|---|---|
| rust is_balanced (rustc -O)                         | 372.5 ± 8.6 ms | `Box` + `&`-borrow |
| c    is_balanced (clang -O3)                        | 392.6 ± 8.6 ms | `*Node` |
| go   is_balanced (GC `*Node`)                       | 397.2 ± 5.7 ms | `*Node` |
| rust is_balanced (rustc -O, overflow-checks=on)      | 422.3 ± 5.5 ms | `Box` + `&`-borrow |
| **kāra is_balanced**                                | **616.1 ± 4.8 ms** | **`shared` (RC)** |

**This is the honest holdout of the traversal sweep — the one shape the RC-elision fix ([kara `B-2026-07-15-21`](https://github.com/karalang/kara/blob/main/docs/bug-ledger.jsonl)) does *not* reach.** Its siblings [#100](../../1-100/100-same-tree/) / [#104](../104-maximum-depth-of-binary-tree/) / [#111](../111-minimum-depth-of-binary-tree/) / [#112](../112-path-sum/) all closed to Rust parity once the per-node refcount traffic was elided; `is_balanced` still runs ~1.46× behind the equal-safety Rust row (422 ms) because its `check` helper stays on the balanced-RC path. **Why it's excluded:** the elision is conservative and fail-closed — condition 1 requires *every* call site to pass the param a **projection** of a named binding, and the thin wrapper `fn is_balanced(root) -> bool { check(root) != -1 }` calls `check(root)` with a **bare identifier** (`root`), which the analysis must treat as a move, not a borrow. That disqualifies `check` entirely, so its by-value `Option[shared]` walk still emits **two retains + two releases per visited node** — the refcount traffic the other four shed. (An earlier draft of this README wrongly called RC "free / already elided" here; the object code shows the `incq`/`decq` pairs are still there.) Closing this would need an interprocedural relaxation that recognizes a bare param forwarded to a borrowed position as a borrow, not a move — a real follow-on, not a fundamental limit. Until then `is_balanced` is the corpus's standing marker for the un-elided read-only walk, and its ~1.46× is the honest cost of the RC traffic these traversals carry when the elision can't fire. Verified correct **and** valgrind-clean (`0 errors, 0 bytes in use at exit`) over the run. Python is listed at **662 ms but ran K = 150,000 — 1/20 of the compiled iterations** (pure-Python recursion is ~20× slower per rep; timed separately, not cross-checked), so its wall-clock is not comparable.
