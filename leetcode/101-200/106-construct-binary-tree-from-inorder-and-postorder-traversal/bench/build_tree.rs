// Benchmark workload for LeetCode #106 — construct binary tree (inorder+postorder), Rust
// mirror (Box). The tree is single-owner (built then dropped, never shared), so Box is the
// honest Rust node. Build 8 (inorder, postorder) pairs once, then K reps of the recursive
// index-bounds reconstruction on a data-dependent-selected pair, folding the rebuilt tree's
// serialization into a rolling hash. Linear inorder scan (O(n^2)) for parity.
...
fn build(post: &[i64], ino: &[i64], plo: i64, phi: i64, ilo: i64, ihi: i64) -> Option<Box<Node>> {
    if plo > phi { return None; }
    let rv = post[phi as usize];
    let mid = find_in(ino, ilo, ihi, rv);
    let lsize = mid - ilo;
    Some(Box::new(Node {
        val: rv,
        left: build(post, ino, plo, plo + lsize - 1, ilo, mid - 1),
        right: build(post, ino, plo + lsize, phi - 1, mid + 1, ihi),
    }))
}
...
fn postorder_of(node: &Option<Box<Node>>, out: &mut Vec<i64>) {
    if let Some(n) = node { postorder_of(&n.left, out); postorder_of(&n.right, out); out.push(n.val); }
}
...
fn main() {
    ...
    for t in 0..8 {
        ...
        let (mut ino, mut post) = (Vec::new(), Vec::new());
        inorder_of(&root, &mut ino);
        postorder_of(&root, &mut post);
        posts.push(post);
        inos.push(ino);
    }
    let mut acc: i64 = 1;
    for _ in 0..800000i64 {
        ...
        let rebuilt = build(&posts[idx], &inos[idx], 0, (bn - 1) as i64, 0, (bn - 1) as i64);
        let s = ser(&rebuilt, 0);
        acc = (acc * 131 + s) % MOD;
    }
    println!("{}", acc);
