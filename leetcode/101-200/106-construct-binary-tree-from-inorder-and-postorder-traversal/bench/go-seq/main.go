// Benchmark workload for LeetCode #106 — construct binary tree (inorder+postorder), Go mirror.
// Build 8 (inorder, postorder) pairs once, then K reps of the recursive index-bounds
...
func build(post, in []int64, plo, phi, ilo, ihi int64) *Node {
	if plo > phi {
		return nil
	}
	rv := post[phi]
	...
	return &Node{
		val:   rv,
		left:  build(post, in, plo, plo+lsize-1, ilo, mid-1),
		right: build(post, in, plo+lsize, phi-1, mid+1, ihi),
	}
}
...
func postorderOf(n *Node, out *[]int64) {
	if n == nil {
		return
	}
	postorderOf(n.left, out)
	postorderOf(n.right, out)
	*out = append(*out, n.val)
}
