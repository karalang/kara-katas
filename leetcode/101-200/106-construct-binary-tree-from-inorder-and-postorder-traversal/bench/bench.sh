#!/usr/bin/env bash
# Wall-clock comparison across implementations of LeetCode #106.
# See ../README.md § Benchmarks for what these numbers mean.
#
# Build 8 (inorder, postorder) input pairs once (15-node trees, distinct shapes), then
# K = 800,000 reps of the recursive index-bounds reconstruction on a DATA-DEPENDENT-
# selected pair (idx = acc%8, seeded by the running hash so nothing hoists), folding the
# rebuilt tree's shape+value serialization into a rolling hash. Each rep allocates a fresh
# 15-node tree and frees it, so the workload is ALLOCATION-bound. Each mirror uses its
...
# timing. Over K=800000 passes the fold is a fixed constant; Python runs a
...
rt_cmd --lang python --approach build_tree --lane seq --mode interp \
...
ce_cmd --lang c --approach build_tree --mode native \
...
echo
echo "=== binary size ==="
size_put --lang kara --approach build_tree --lane seq --mode codegen --path "target/${STEM}_kara"
size_put --lang rust --approach build_tree --lane seq --mode native  --path "target/${STEM}"
size_put --lang c    --approach build_tree --lane seq --mode native  --path "target/${STEM}_c"
size_put --lang go   --approach build_tree --lane seq --mode native  --path "target/${STEM}_go_seq"

echo
echo "=== runtime memory (peak) ==="
mem_put --lang kara --approach build_tree --lane seq --mode codegen --bytes "$(mem_peak ./target/${STEM}_kara)"
mem_put --lang rust --approach build_tree --lane seq --mode native  --bytes "$(mem_peak ./target/${STEM})"
mem_put --lang c    --approach build_tree --lane seq --mode native  --bytes "$(mem_peak ./target/${STEM}_c)"
mem_put --lang go   --approach build_tree --lane seq --mode native  --bytes "$(mem_peak ./target/${STEM}_go_seq)"
