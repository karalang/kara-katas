...
bench_begin id=186 slug=reverse-words-in-a-string-ii group=101-200 \
    title="Reverse Words in a String II" \
    workload="~30K-char word buffer (i64 codes) built once, 3000 in-place two-reversal word-reversal passes with a per-pass char punch; sink=sum of per-pass Horner checksums of the buffer bytes" sink="$expected"
