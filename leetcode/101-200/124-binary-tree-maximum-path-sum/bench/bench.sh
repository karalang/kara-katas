#!/usr/bin/env bash
# Wall-clock comparison across implementations of LeetCode #124.
...
bench_begin id=124 slug=binary-tree-maximum-path-sum group=101-200 \
    title="Binary Tree Maximum Path Sum" \
    workload="post-order clamp-DP: gain(c)=max(0,up(c)), through=val+gain(L)+gain(R), answer=max through over all nodes, threaded in a mut ref i64; forest of T=2048 balanced mixed-sign trees of N=511 RC nodes built ONCE, K=60 serial passes folding a sum-of-results sink" sink="$expected"
