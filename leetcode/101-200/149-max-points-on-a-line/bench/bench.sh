#!/usr/bin/env bash
# Wall-clock comparison across implementations of LeetCode #149.
...
build_kara() {
    ...
    # SEQ lane: KARAC_AUTO_PAR=0 for a fair single-threaded comparison against
    # the C/Rust/Go seq mirrors. The independent-anchor outer loop is
    # embarrassingly parallel, so the DEFAULT build auto-parallelizes it — that
    # is measured separately as the `kara_par` lane below, never mixed into the
    # seq comparison.
...
build_kara_par() {
    ...
    # AUTO-PAR lane: the DEFAULT build, which auto-parallelizes the anchor loop.
    if [ ! -x "$out" ] || [ "$src" -nt "$out" ] || [ "$(command -v karac)" -nt "$out" ]; then
...
bench_begin id=149 slug=max-points-on-a-line group=101-200 \
    title="Max Points on a Line" \
    workload="sort-based slope-run count, N=1200 K=8" sink="$expected"

echo "=== runtime — short workloads (compiled) ==="
rt_begin --warmup 5 --runs 30
rt_cmd --lang kara --approach "$STEM" --lane seq --mode codegen \
    --name "kara ${STEM} (codegen, seq)" --cmd "./target/${STEM}_kara"
rt_cmd --lang kara --approach "$STEM" --lane par --mode codegen \
    --name "kara ${STEM} (codegen, auto-par)" --cmd "./target/${STEM}_kara_par"
...
mem_put --lang kara --approach "$STEM" --lane par --mode codegen --bytes "$(mem_peak ./target/${STEM}_kara_par)"
