...
**MANDATORY when auto-par fires (implicit OR explicit): build the par-lane comparators.** A legitimate auto-par win is the corpus's single most important commercial result — *Kāra got a parallel speedup with no parallel code*. Reporting Kāra's auto-par wall time **in isolation, against nothing**, throws that away: a reader can't tell whether it beats what a Rust/Go programmer would hand-write. So whenever the kara binary picks up `karac_par_reduce` (`nm -gU … | grep karac_par_reduce`), the kata **must** ship four par rows comparing the SAME reduction:

- **Kāra (auto-par)** — the default `karac build` binary; NO parallel source.
- **C + pthreads** (`<approach>_par.c`, `clang -O3 … -lpthread`) — the **bare-metal FLOOR**: a fixed pool of `_SC_NPROCESSORS_ONLN` threads (spawn once, chunk the range, join+merge), raw OS threads with no runtime/work-stealing/GC. NOT an ergonomic competitor — it calibrates how much auto-par leaves on the table vs metal, exactly as `clang -O3` is the seq-lane codegen floor. (pthreads not OpenMP — Apple clang has no `-fopenmp`/libomp by default, and raw threads are the truer "metal".)
- **Rust + rayon** (`bench/rayon/`, own Cargo project, `.into_par_iter()`) and **Go goroutines** (`bench/go-par/`, chunk + `WaitGroup` + partial-merge) — the *ergonomic competitors* a programmer reaches for.

The headline is *Kāra-auto-par vs the C metal floor vs hand-tuned rayon/goroutines* — within the par lane — plus the buyer reframe (colorless parallelism: fewer lines, no `Send`/`Sync`/data-race reasoning, near-floor throughput). #722 and #394 are the worked examples, and they show the floor cuts both ways: on #722's **chunky** tasks the C floor + rayon (≈16.6 ms) are genuinely fastest and Kāra (18.3 ms) has ~10% headroom; on #394's **fine-grained** tasks the per-process pthread spawn + bandwidth contention make pooled schedulers win, so Kāra's auto-par (72 ms) **beats** the C floor (83 ms) and rayon (85 ms). Report whichever it is honestly — the C floor is what makes "how much headroom is left" legible. A Kāra-only "auto-par regime" row with no comparators is an incomplete kata.
