...
**MANDATORY when auto-par fires (implicit OR explicit): build the par-lane comparators.** A legitimate auto-par win is the corpus's single most important commercial result — *Kāra got a parallel speedup with no parallel code*. Reporting Kāra's auto-par wall time **in isolation, against nothing**, throws that away: a reader can't tell whether it beats what a Rust/Go programmer would hand-write. So whenever the kara binary picks up `karac_par_reduce` (`nm -gU … | grep karac_par_reduce`), the kata **must** ship `bench/rayon/` (`.into_par_iter()`) **and** `bench/go-par/` (goroutines + `WaitGroup` + partial-merge), both parallelizing the SAME reduction, and the bench.sh par section must compare all three. The headline is *Kāra-auto-par vs hand-tuned-rayon vs hand-tuned-goroutines* — within the par lane — plus the buyer reframe (colorless parallelism: fewer lines, no `Send`/`Sync`/data-race reasoning, equal-or-better throughput). #722 and #394 are the worked examples (#394: auto-par 72 ms beats rayon 85 ms with zero parallel source). A Kāra-only "auto-par regime" row with no comparators is an incomplete kata.
